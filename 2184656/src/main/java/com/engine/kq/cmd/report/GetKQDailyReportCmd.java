...
import com.api.customization.kq.report.util.KqCardSignUtil;
...
//			}
			...
			while (rs.next()) {
				...
				while (kqReportFieldComInfo.next()) {
					...
					if (fieldName.equals("subcompany")) {
						...
						KqCardSignUtil kqCardSignUtil = new KqCardSignUtil();
						fieldValue = kqCardSignUtil.getCardCount(id,kqdate,kqdate);
						data.put(fieldName,fieldValue);
					}else {
						fieldValue = Util.null2String(rs.getString(fieldName));
	...
	public Map<String, Object> getSignDetailInfo(String resourceId, String kqDate){
        ...
        boolean isCardIn = KqSignCusBiz.isCardSign(signinid);
        boolean isCardOut = KqSignCusBiz.isCardSign(signoutid);
            ...
            signStatusInfo.put("forgotBeginWorkCheckMins",forgotBeginWorkCheckMins);
                        ...
                        if (isCardIn){
                            data.put("signintime"+serialnumber,KqSignCusBiz.getNoCardSign(resourceId,kqdate,1,serialnumber-1));
                            data.put("signinstatus"+serialnumber,"已补卡");
                        }
					}
					...
					if (workendtime.length() > 0) {
                        ...
                        if (isCardOut){
                            data.put("signouttime"+serialnumber,KqSignCusBiz.getNoCardSign(resourceId,kqdate,2,serialnumber-1));
                            data.put("signoutstatus"+serialnumber,"已补卡");
                        }
					}
				}else{
					if(workMins>0){
