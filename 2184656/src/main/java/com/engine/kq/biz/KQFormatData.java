...
public class KQFormatData extends BaseBean {
  ...
  //==zj 计算开启流程抵扣打卡，但流程不在打卡时间内，计算漏签次数
  static int forgotbeginworkcheck = 1;
  ...
  public Map<String, Object> formatKqDate(String userId, String kqDate) {
    new BaseBean().writeLog("==zj==(userId)" + userId);
    ...
    try {
      ...
      if (workTime == null || workTime.getWorkMins() == 0) {
        ...
        if (lsParam.size() > 0) {
          ...
          new BaseBean().writeLog("==zj==(统计漏签次数)" + forgotbeginworkcheck);
          sql = " insert into kq_format_total(resourceid,kqdate,subcompanyid,departmentid,jobtitle,groupid,serialid,workdays,workmins," +
              ...
              " sum(case when graveLeaveEarlyMins> 0 then 1 else 0 end) as graveLeaveEarly,sum(graveLeaveEarlyMins) as graveLeaveEarlyMins, " +
              " sum(case when absenteeismmins> 0 then 1 else 0 end) as absenteeism,sum(absenteeismmins) as absenteeismmins," +
              " sum(case when forgotcheckmins> 0 then "+forgotbeginworkcheck+" else 0 end) as forgotcheck,sum(forgotcheckmins) as forgotcheckmins,sum(leaveMins) as leaveMins," +
    ...
    try {
      ...
      for (int i = 0; lsWorkTime != null && i < lsWorkTime.size(); i++) {
        ...
        if(oneSign){
          ...
          new BaseBean().writeLog("==zj=(lsSignTime)" + JSON.toJSONString(lsSignTime));
          ...
          KqSignCusBiz.upDeductCardWorkTime(userId,kqDate,workBeginTime,workEndTime);
        }
        ...
        for (int j = 0; lsCheckInfo != null && j < lsCheckInfo.size(); j++) {
          ...
          new BaseBean().writeLog("==zj==(checkInfo)" + JSON.toJSONString(checkInfo));
          new BaseBean().writeLog("==zj==(流程抵扣)" + deduct_signintime + " | " + deduct_signofftime);
          ...
          if (checkInfo.get("signType").equals("1")) {//签到
            if(signTime.length() > 0){
              //09:00:01
               signTime = signTime.substring(0,5)+":00";   //09:00:00
              new BaseBean().writeLog("==zj==(signTime)" +signTime);
        ...
        //==zj 因为考勤流程没有覆盖到流程打卡时间，所以直接计为旷工时长
        new BaseBean().writeLog("==zj==(计算旷工时长)" + workBeginIdx + " | " +workEndIdx);
        new BaseBean().writeLog("==zj==(签到和签出次数)" + checkIn +  " | " + checkOut);
        String flow_deduct_card = "2";    //0：有流程，但未开启抵扣打卡 1：有流程，并开启抵扣打卡 2：没有流程，也没有开启抵扣打卡
        ...
        if (checkIn == 0 && checkOut == 0) {//旷工(无签到无签退)
          ...
          String kqDateNext = DateUtil.addDate(kqDate, 1);
          KQFlowDataBiz kqFlowDataBiz = new KQFlowDataBiz.FlowDataParamBuilder().resourceidParam(userId).fromDateParam(kqDate).toDateParam(kqDateNext).build();
          List<SplitBean> allFlowData = kqFlowDataBiz.getAllFlowData(workFlowInfo, false);
          kqLog.info("查看考勤流程数据" + JSON.toJSONString(allFlowData));


          //== zj 获取到流程数据
          if (allFlowData != null && allFlowData.size() > 0){
            SplitBean splitBean = allFlowData.get(0);
            if (!"".equals(splitBean.getRequestId())){
              workflowId = splitBean.getWorkflowId();
              new BaseBean().writeLog("==zj==(获得到的workflowid)" + workflowId);

              //==zj 根据考勤流程的workflowid,来查询是否开启流程抵扣打卡记录开关
              if (workflowId!="" && workflowId!=null){
                KQAttProcSetComInfo kqAttProcSetComInfo = new KQAttProcSetComInfo();
                flow_deduct_card = Util.null2String(kqAttProcSetComInfo.getFlow_deduct_card(""+workflowId));
              }
        ...
        new BaseBean().writeLog("==zj==(workFlow)" + JSON.toJSONString(workFlow));
        ...
        new BaseBean().writeLog("==zj==(workBeginIdx)" + workBeginIdx +" | " + workEndIdx);
        new BaseBean().writeLog("==zj==(dayMins)" + JSON.toJSONString(dayMins));
        ...
        //==zj== 如果流程有开启抵扣打卡记录
        new BaseBean().writeLog("==zj==(查看flow_deduct_card状态)" +flow_deduct_card);
        if ("1".equals(flow_deduct_card)){
          //==zj== 当flow_deduct_card=1时，说明有流程抵扣打卡，但是因为流程没有包含在打卡时间，所以走checkin=0,checkout=0
          //并且如果有请假时长 或 出差时长 或公出时长 说明该打卡抵扣流程是有在工作时长内
          if (leaveMins!=0 || evectionMins!=0 || outMins!=0){
            forgotCheckMins = absenteeismMins;    //将旷工时长转为漏签时长
            forgotBeginWorkCheckMins = absenteeismMins;   //将旷工时长转为漏签开始时长
            absenteeismMins = 0;                          //将旷工时长归0

            HashMap<String, Integer> logMap = new HashMap<String,Integer>();
            logMap.put("forgotCheckMins",forgotCheckMins);
            logMap.put("forgotCheckMins",forgotBeginWorkCheckMins);
            logMap.put("absenteeismMins",absenteeismMins);
            new BaseBean().writeLog("==zj==(时长转换logMap)" + JSON.toJSONString(logMap));
        ...
        //zj
        //计算实际出勤时间(出差公出算出勤)=应出勤-旷工-迟到-早退
        attendanceMins = workMins - absenteeismMins-beLateMins-graveBeLateMins-leaveEarlyMins-graveLeaveEarlyMins;
        //修改实际出勤计算不扣减请假时长-2274929
        //一天一班次时，漏签一次，应出勤时长减少半个班次的时长，漏签两次扣钱
        if (oneSign){
          if (forgotCheckMins>0){
            forgotCheckMins = (int) (workMins*0.5);
          }
          if (forgotBeginWorkCheckMins>0){
            forgotBeginWorkCheckMins = (int) (workMins*0.5);
          }
        ...
        kqLog.info("实际出勤计算公式" + "实际出勤=应出勤- 旷工-迟到-早退-漏签（漏签=次数*0.5*应出勤时长）  userId" + userId + "kqDate==" + kqDate+":hostIps:"+hostIps+":uuid::"+uuid);
        kqLog.info("实际出勤计算结果" + attendanceMins + "=" + workMins + "- " + absenteeismMins + "-" + leaveMins + "-" + (beLateMins + graveBeLateMins) + "-" + (leaveEarlyMins - graveLeaveEarlyMins)+"-"+ (forgotBeginWorkCheckMins+forgotCheckMins)+" userId" + userId + "kqDate==" + kqDate+":hostIps:"+hostIps+":uuid::"+uuid);
        if(this.writeLog) {
...
//          System.out.println("fieldname=="+fieldname);
      ...
      writeLog(e);
      kqLog.info(e);
      new BaseBean().writeLog("==zj==(format)" + e);
