...
public class KQFlowDataBiz {
  ...
  public List<SplitBean> getLeaveData(Map<String, Object> flowMaps,
    ...
    new BaseBean().writeLog("==zj==(请假流程sql)" + leaveSql);
    rs.execute(leaveSql);
    while(rs.next()){
      ...
      String workflowid = Util.null2String(rs.getString("workflowid"));       //获取请假流程workflowid

      //==zj 将请假流程数据放入splitBean里
      ...
      splitBean.setLeavebackrequestid(leavebackrequestid);
  ...
  public List<SplitBean> getEvectionData(Map<String, Object> flowMaps,
    ...
    new BaseBean().writeLog("==zj==(出差流程sql)" + leaveSql);
    rs.execute(leaveSql);
    while(rs.next()){
      SplitBean splitBean = new SplitBean();
      String requestId= rs.getString("requestId");
      ...
      String duration= rs.getString("duration");
      String durationrule= rs.getString("durationrule");
      String workflowid = Util.null2String(rs.getString("workflowid"));   //获取出差流程的workflowid
      ...
      splitBean.setDuration(duration);
      splitBean.setDurationrule(durationrule);
      splitBean.setWorkflowId(workflowid);        //设置出差流程的workflowid
      splitBeans.add(splitBean);
  ...
  public List<SplitBean> getOutData(Map<String, Object> flowMaps,
    ...
    List<SplitBean> splitBeans = new ArrayList<>();
    new BaseBean().writeLog("==zj==(公出sql)" + leaveSql);
    rs.execute(leaveSql);
    while(rs.next()){
      SplitBean splitBean = new SplitBean();
      String requestId= rs.getString("requestId");
      ...
      String duration= rs.getString("duration");
      String durationrule= rs.getString("durationrule");
      String workflowid = Util.null2String(rs.getString("workflowid"));   //获取workflowid
      new BaseBean().writeLog("==zj==(获取公出流程workflowid)" + workflowid);
      ...
      splitBean.setDuration(duration);
      splitBean.setDurationrule(durationrule);
      splitBean.setWorkflowId(workflowid);            //赋值workflowid
      splitBeans.add(splitBean);
