  ...
  public void buildOvertime(String resourceid, String fromDate, String toDate, String eventtype){
    ...
    try{
      ...
      for (int i = 0; i <= betweenDays; i++) {
        ...
        new BaseBean().writeLog("==zj==(computingMode)" +computingMode);
        if(computingMode == 2){
...
//        需审批，以打卡为准，取流程和打卡的交集
  ...
  private void doComputingMode4(String resourceid, String splitDate,
    ...
    new BaseBean().writeLog("==zj==(加班时长方法交集进入)");
    new BaseBean().writeLog("==zj==(lsCheckInfoMaps)" + JSON.toJSONString(lsCheckInfoMaps));
    ...
    while (iterator.hasNext()) {
      ...
      new BaseBean().writeLog("==zj==(kqOvertimeCardBean)" +JSON.toJSONString(kqOvertimeCardBean));
      if(kqOvertimeCardBean != null){
        ...
        //==zj buildOvertimeCard
        KQOvertimeRulesDetailEntity curKqOvertimeRulesDetailEntity = overRulesDetailMap.get(changeType_key);  //获取实体
        Map<String,String> signinoffMap = buildOvertimeCard(kqOvertimeCardBean, resourceid, realSplitDate, kqTimesArrayComInfo, restTimeMap, changeType_key,initArrays,hasOverTime4SignList,
            overRulesDetailMap,true,overtimeLogMap,curKqOvertimeRulesDetailEntity);
        ...
        if(signinTimeIndex < signoutTimeIndex){
          //先覆盖打卡 打卡区间都是1
          new BaseBean().writeLog("==zj==(initArrays)" + JSON.toJSONString(initArrays));
          ...
          new BaseBean().writeLog("==zj==(over_count)" +over_count);
          if(over_count > 0){
            int restTimeType = 1;
            ...
            if(kqOvertimeRulesDetailEntity != null){
              ...
              new BaseBean().writeLog("==zj==(over_count和minimumLen)" + over_count + " | " + minimumLen);
...
//              cross_time_list里存的是排除了工作时间的打卡段，找到1表示找到打卡开始的点了，找到-2表示找到打卡结束的点了
                ...
                new BaseBean().writeLog("==zj==(交集前)" + JSON.toJSONString(initArrays_flow));
                Arrays.fill(initArrays_flow, flow_fromIndex, flow_toIndex,2);
                new BaseBean().writeLog("==zj==(交集后)" + JSON.toJSONString(initArrays_flow));
...
//                }
                ...
                for(int i = 0 ; i < cross_time_list.size() ;) {
                  ...
                  if(flow_count > 0){
                    ...
                    new BaseBean().writeLog("==zj==(流程开始-流程结束)" + JSON.toJSONString(flow_cross_time_list) + " | " + JSON.toJSONString(initArrays_flow));
                    logOvertimeMap(overtimeLogMap, flow_cross_time_list, flow_key+"|取打卡和流程相交的区间|flow_cross_time_list");
                    new BaseBean().writeLog("==zj==(取打卡和流程相交的区间)" + JSON.toJSONString(flow_cross_time_list));
                ...
                //==zj 根据加班规则id,获取“是否排除午休时间段”开关状态，如果没有开启，午休时间也算加班时长
                /*int ruleId =  kqOvertimeRulesDetailEntity.getRuleId();   //获取加班规则id
                int dayType = kqOvertimeRulesDetailEntity.getDayType(); //获取日期类型。1-节假日、2-工作日、3-休息日
                RecordSet rw = new RecordSet();

                String baseSql = "select a.hasRestTime,b.startTime,b.endTime from kq_overtimeRulesDetail a left join kq_OvertimeRestTime b on a.ruleId = b.ruleId";
                String whereSql=" where a.ruleId=" + ruleId + " and a.dayType=" + dayType;
                baseSql += whereSql;
                new BaseBean().writeLog("==zj==(查询开关状态，和午休时间段)" +baseSql);

                rw.executeQuery(baseSql);
                if (rw.next()){
                  int hasResTime = rw.getInt("hasRestTime");  //获取对应日期类型的开关状态
                  //如果是关闭状态
                  if (hasResTime == 0){
                    List<TimeScopeEntity> restTime = workTimeEntity.getRestTime();  //获取午休数据list
                    TimeScopeEntity timeScopeEntity = restTime.get(0);              //获取午休实体
                    int beginTime = kqTimesArrayComInfo.getArrayindexByTimes(timeScopeEntity.getBeginTime()); //将午休开始时间转为分钟
                    int endTime = kqTimesArrayComInfo.getArrayindexByTimes(timeScopeEntity.getEndTime()); //将午休结束时间转为分钟
                    int sumTime = kqTimesArrayComInfo.getCnt(initArrays_flow, beginTime, endTime, 2); //获取流程交集部分，将午休开始时间到结束时间范围，计算多少个2.
                    new BaseBean().writeLog("==zj==(获取午休时长)" + sumTime);
                    across_mins +=sumTime;
                  }
                }*/
                //==
                ...
                if(needSplitByTime){
                  ...
                  new BaseBean().writeLog("==zj==(最终生成的加班分钟数-修改1)" + across_mins);
    ...
    while (iterator.hasNext()) {
      ...
      if(kqOvertimeCardBean != null){
        ...
        Map<String,String> signinoffMap = buildOvertimeCard(kqOvertimeCardBean, resourceid, realSplitDate, kqTimesArrayComInfo, restTimeMap, changeType_key,initArrays,Lists.newArrayList(),overRulesDetailMap,
            true, overtimeLogMap,curKqOvertimeRulesDetailEntity);
...
//                生成过加班，但是其实只用了1小时，还需要拿这个加班流程来，减去已经用的1小时，d_mins就是这个加班流程实际可用的加班流程时长
            ...
            if(d_mins > 0){
              ...
              Map<String,String> signinoffMap = buildOvertimeCard(kqOvertimeCardBean, resourceid, realSplitDate, kqTimesArrayComInfo, restTimeMap, changeType_key,initArrays,hasOverTimeList,
                  overRulesDetailMap, false, overtimeLogMap,curKqOvertimeRulesDetailEntity);
...
//                    }
  ...
  public Map<String,String> buildOvertimeCard(KQOvertimeCardBean kqOvertimeCardBean,
      ...
      Map<String, Object> overtimeLogMap,KQOvertimeRulesDetailEntity curKqOvertimeRulesDetailEntity) throws Exception{
    ...
    if(restTimeMap.containsKey(changeType_key)){
      ...
      //再把休息时间填充上去
      new BaseBean().writeLog("==zj==(restTimeList)" + JSON.toJSONString(restTimeList));
      if(!restTimeList.isEmpty()){
        int restTimeType = 1;
        new BaseBean().writeLog("==zj==(restTimeType)" + restTimeType);
        if(kqOvertimeRulesDetailEntity != null){
          restTimeType = kqOvertimeRulesDetailEntity.getRestTimeType();
        }
        if(restTimeType == 1){
          ...
          if(lsRestTime != null && !lsRestTime.isEmpty()){
            ...
            for(int p = 0 ; p < lsRestTime.size(); p++){
              ...
              if(begintimeIndex < endtimeIndex){
                ...
                //zj排除休息时间
                int hasRestTime = kqOvertimeRulesDetailEntity.getHasRestTime();
                new BaseBean().writeLog("==zj==(kqOvertimeRulesDetailEntity打卡规则)" + JSON.toJSONString(kqOvertimeRulesDetailEntity));
                new BaseBean().writeLog("==zj==(排除休息时间-hasRestTime)" + hasRestTime + " | " + begintimeIndex + " | " + endtimeIndex);
                if (hasRestTime == 0){
  ...
  public List<OvertimeBalanceTimeBean> getOvertimeBalanceTimeBean(String resourceId,
    ...
    if(changeType == 1){
      ...
      //==zj使用相同方法，放一个空的参数进去
      KQOvertimeRulesDetailEntity curKqOvertimeRulesDetailEntity = null;
      boolean isok = kqOverTimeFlowBiz.handle_changeType_2(initArrays, resourceId, fromdate, before_startTime, startTime, fromTime_index,
          kqTimesArrayComInfo, splitBean,toTime_index,curKqOvertimeRulesDetailEntity);
