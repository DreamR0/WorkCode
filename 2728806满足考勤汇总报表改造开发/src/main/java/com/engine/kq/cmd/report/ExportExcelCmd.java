...
//      }
      ...
      //==z 这里额外添加自定义个导出列
      kqFormatCustomUtil.setCustomCol(titleList);
      ...
      String backFields = " a.id,a.lastname,a.workcode,a.dsporder,b.resourceid,a.subcompanyid1 as subcompanyid,a.departmentid,a.jobtitle," +
              ...
              "sum(b.absenteeismDays) as absenteeismDays, sum(b.nightshiftBig) as nightshiftBig,sum(b.nightShiftSmall) as nightShiftSmall,"+
      ...
      while (rs.next()) {
        ...
        for (int fieldDataKeyIdx =0;fieldDataKeyIdx<lsFieldDataKey.size();fieldDataKeyIdx++) {
          ...
          if (fieldName.equals("subcompany")) {
          ...
          }else if(fieldName.equals("workOverTimeReal")){
            //计算实际工作日加班时长
            double total = 0.0;
            ...
            total = workingDayOvertime_4leave+workingDayOvertime_nonleave;
            if (total >= 36){
              total = 36;
            }else if (total <= 0){
              total=0.0;
            ...
            fieldValue = total+"";
          }else if(fieldName.equals("overTimeDaysExtras")){
            //计算工作日之外的加班天数（星期六），最多算4天
            fieldValue = kqFormatCustomUtil.getOverTimeDayExtras(id,fromDate,toDate);
...
//          Map<String, Object> detialDatas = kqReportBiz.getDetialDatas(id, fromDate, toDate, user,flowData,true);
          ...
          //缺勤日期
          String fieldValue = "";
          fieldValue = kqFormatCustomUtil.getAbsenteeismDates(id,fromDate,toDate);
          data.add(fieldValue);
          isHave = true;
        }
        if (!isHave){
      ...
      new BaseBean().writeLog("==zj==(workbook)" + JSON.toJSONString(workbook));
