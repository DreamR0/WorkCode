...
import com.api.cusomization.qc2728806.util.KqCustomDetail;
...
public class KQReportDetailAction extends BaseBean {
	...
	public String getKQReportDetail(@Context HttpServletRequest request, @Context HttpServletResponse response) {
		...
		KqCustomDetail kqCustomDetail = new KqCustomDetail();
		...
		try {
			...
			}else if(type.equals("absenteeismDays")){
				apidatas = kqCustomDetail.getAbsenteeismDaysDetail(params,user);
			}else if ((type.equals("nightshiftBig"))){
				apidatas = kqCustomDetail.getNightShiftBigDetail(params,user,0);
			}else if ((type.equals("nightshiftSmall"))){
				apidatas = kqCustomDetail.getNightShiftBigDetail(params,user,1);
			}
		} catch (Exception e) {
			apidatas.put("status", "-1");
			writeLog(e);
