...
import com.api.cusomization.qc2728806.util.KqFormatCustomUtil;
...
public class KQFormatData extends BaseBean {
      ...
      if (workTime == null || workTime.getWorkMins() == 0) {
        ...
        if (lsParam.size() > 0) {
          sql = " insert into kq_format_detail( " +
                  ...
                  " leaveMins,leaveinfo,evectionMins,outMins,forgotbeginworkcheckmins,absenteeismDays,nightshiftBig,nightShiftSmall,otherinfo"+(definedField.length()>0?","+definedField+"":"")+") " +
          ...
          sql = " insert into kq_format_total(resourceid,kqdate,subcompanyid,departmentid,jobtitle,groupid,serialid,workdays,workmins," +
                  ...
                  " leaveMins,evectionMins,outMins,forgotbeginworkcheck,forgotbeginworkcheckmins,absenteeismDays,nightshiftBig,nightShiftSmall"+(definedField.length()>0?","+definedField+"":"")+") " +
                  " select a.resourceid,kqdate,b.subcompanyid1,b.departmentid,b.jobtitle,groupid,serialid," +
                  " case when sum(workmins)>0 then 1 end as workdays, sum(workmins) as workmins," +
                  ...
                  "sum(absenteeismDays) as absenteeismDays, sum(nightshiftBig) as nightshiftBig, sum(nightShiftSmall) as nightShiftSmall" +
  ...
  public List<List<Object>> format(String userId, String kqDate, WorkTimeEntity workTime,
    ...
    KqFormatCustomUtil kqFormatCustomUtil = new KqFormatCustomUtil();
    try {
...
//        }
        ...
        //==z 如果当天有一次没打卡就算缺勤一天,但是如果有出差或者公出就不算缺勤
        int absenteeismDays = 0;
        if (kqFormatCustomUtil.isAbsenteeism(checkIn,checkOut,absenteeismMins,userId,kqDate)){
          absenteeismDays = 1;
        }
        //==z 计算当天班次是否为大夜班
        int isNightShiftBig = 0;
        if (kqFormatCustomUtil.isNightShiftBig(kqDate,userId)){
          isNightShiftBig = 1;
        }

        //==z 计算当天是否为小夜班，如果有大夜班就不计算
        int nightShiftSmall = 0;
        if (signOutTime.compareTo("20:00") > 0 && isNightShiftBig==0 ){
          nightShiftSmall = 1;
        }
        ...
        params.add(isNightShiftBig);
        params.add(nightShiftSmall);
