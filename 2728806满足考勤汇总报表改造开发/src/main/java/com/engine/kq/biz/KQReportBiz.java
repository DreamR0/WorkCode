...
//      Map<String, Object> data = new HashMap<>();
  ...
  public Map<String,Object> getFlowOverTimeDataNew(Map<String,Object> params, User user){
    ...
    try{
      ...
      DecimalFormat decimalFormat = new DecimalFormat("#0.0");//加班时长精确到小数点1位
      ...
      while (rs.next()) {
        ...
        value =Util.getDoubleValue(decimalFormat.format(value)) ;
        ...
        if(datas.containsKey(key)){
          ...
          tmpVal += value;
          datas.put(key,/*format(tmpVal)*/tmpVal);
        }else{
          datas.put(key,/*format(value)*/value+"");
        }
      }
    }catch (Exception e){
      writeLog(e);
