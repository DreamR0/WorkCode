...
import com.api.customization.qc2521698.util.AmountUtil;
...
public class KQBalanceOfLeaveBiz {
    ...
    public static BigDecimal getCanUseAmount(String resourceId, String ruleId, String belongYear, BigDecimal baseAmount, String legalOrWelfare, String date) {
        ...
        try {
            ...
            //qc2521698这里对年假类型进行判断，新员工则按月释放，老员工按年释放
            AmountUtil amountUtil = new AmountUtil();
            Boolean isYearLeave = amountUtil.isYearLeave(ruleId);
            new BaseBean().writeLog("==zj==(isYearLeave)" + JSON.toJSONString(isYearLeave));
            if (isYearLeave){
                //如果是年假就控制释放规则 0--按年 1--按天 2--按月
                releaseRule = amountUtil.getReleaseRule(resourceId,companyStartDate,ruleId);
            }
            /* 释放规则 start */
            if (releaseRule == 0) {
            ...
            new BaseBean().writeLog("==zj==(假期释放规则)" +releaseRule + "  |  "  + resourceId + "   |   " + JSON.toJSONString(canUseAmount));
            /* 释放规则 end */
            //如果释放为负数，就修改为0
