                        ...
                        for (Entry<String, Object> typeEntry : keave_set) {
                            ...
                            if (balanceEnable) {
                                if ("1".equalsIgnoreCase(isFormal)) {
                                    ...
                                    if (isTiaoxiu){
                                        balanceOfLeave = kqCustomUtil.getRestAmountMonth(resourceId, newLeaveType, DateUtil.getCurrentDate(),false,true,reduceMonth);
                                    }else {
                                        balanceOfLeave = KQBalanceOfLeaveBiz.getRestAmount(resourceId, newLeaveType, DateUtil.getCurrentDate());
                                    }
                                } else {
                                    if (isTiaoxiu){
                                        balanceOfLeave = kqCustomUtil.getRestAmountMonth(resourceId, newLeaveType, date,false,true,reduceMonth);
                                    }else {
