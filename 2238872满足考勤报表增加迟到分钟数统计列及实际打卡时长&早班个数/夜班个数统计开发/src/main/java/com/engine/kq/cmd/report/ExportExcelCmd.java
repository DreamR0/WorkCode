    ...
    try {
      ...
      for(String showColumn:showColumns){
        ...
        if(showColumn.equals("beLate")){
          cascadekey = "beLateMins,beLateMinsD";
        }else if(showColumn.equals("leaveEearly")){
          cascadekey = "leaveEarlyMins,leaveEarlyMinsD";
...
//      }
      ...
      new BaseBean().writeLog("==zj==(导出报表sql语句)" +sql);
      rs.execute(sql);
      while (rs.next()) {
        data = new ArrayList<>();
        ...
        for (int fieldDataKeyIdx =0;fieldDataKeyIdx<lsFieldDataKey.size();fieldDataKeyIdx++) {
          ...
          if (fieldName.equals("subcompany")) {
            ...
            for(int i=0;i<lsCascadekey.size();i++){
              if(Util.null2String(rs.getString(lsCascadekey.get(i))).length()>0){
                ...
                if ("leaveEarlyMinsD".equals(lsCascadekey.get(i)) || "beLateMinsD".equals(lsCascadekey.get(i))){
                  //==zj 这里获取早退时长和迟到时长的值
                  new BaseBean().writeLog("==zj==(lsCascadekey)" + JSON.toJSONString(lsCascadekey));
                  new BaseBean().writeLog("==zj==(早退|迟到)" + lsCascadekey.get(i) + " | " + lsCascadekey.get(i-1));
                  if ("leaveEarlyMinsD".equals(lsCascadekey.get(i))){
                    fieldValue = Util.null2String(rs.getString("leaveEarlyMins"));
                  }
                  if ("beLateMinsD".equals(lsCascadekey.get(i))){
            ...
            continue;
              ...
              //==zj 这里修改下实际工作时长的值，按0.5向下取整
                if ("attendanceMins".equals(fieldName)){
                  fieldValue = Util.null2String(rs.getString(fieldName));
                  long value = Math.round(Util.getDoubleValue(fieldValue) / 60.0);
                  fieldValue =  KQDurationCalculatorUtil.getDurationRound("" + value);
                }
          }else if("workdays".equals(fieldName) || "workmins".equals(fieldName)){
            //==zj 这里修改应出勤天数和应工作时长，按照节假日设置显示
          ...
          }else if("overtimePeace".equals(fieldName)){
            //==zj 这里对平时加班字段进行赋值
