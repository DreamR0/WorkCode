    ...
    if(!sqlMap.isEmpty()){
      for(Map.Entry<String,String> me : sqlMap.entrySet()){
        ...
        while (rs1.next()) {
          ...
          //==zj 这里多加一个公出类型进行判断，实现和出差流程同行人的功能
          new BaseBean().writeLog("==zj==(durationTypeEnum)"+durationTypeEnum);
          if(durationTypeEnum == DurationTypeEnum.EVECTION || durationTypeEnum == DurationTypeEnum.OUT){
            String companion = Util.null2s(splitBean.getCompanion(), "");
            new BaseBean().writeLog("==zj==(companion)" + companion);
            if(companion.length() > 0){
