...
//        }
        ...
        String meeting_type = Util.null2String(params.get("meetingtype"));
        String callers = Util.null2String(params.get("caller"));
        new BaseBean().writeLog("会议基本信息提交" + meetingtype + "--" + name + "--" + caller + "--" + contacter + "--" + desc_n);
        //时间
        ...
        //==zj==基本信息提交（测试）
        if (meetingtype != null && !"".equals(meeting_type)){
            updateSql += " , meetingtype = '" + meeting_type + "' ";
        }
        new BaseBean().writeLog("基本信息--会议类型：" + meetingtype);
        if (name != null && !"".equals(name)){
            updateSql += " , name = '" + name + "'";
        }
        if (callers != null && !"".equals(callers)){
            updateSql += " , caller = '" + callers  + "'";
        }
        if (contacter != null && !"".equals(contacter)){
            updateSql += " , contacter = '" + contacter + "'" ;
        }
        if (desc_n != null && !"".equals(desc_n)){
        ...
        updateSql += " , crmmembers = '" + crmmembers + "' "
                ...
                + " where id = " + meetingid;
        //变更会议基本信息
        new BaseBean().writeLog("变更会议sql语句开始执行");
        new BaseBean().writeLog("变更会议sql语句数据：" + updateSql);
        recordSet.execute(updateSql);
        new BaseBean().writeLog("变更会议sql执行结束");
        if (recordSet.getDBType().equalsIgnoreCase("oracle") &&Util.null2String(recordSet.getOrgindbtype()).equals("oracle")) {
        ...
        if (!oldmembers.equals(hrmmembers) || !oldcrmmembers.equals(crmmembers) || meetingInfoChanged) {//前后人员不相同或客户不相同或主要信息发生变更
            ...
            try {
                ...
                if(!oldaddress.equals(address1)){
                    ...
                    rs.executeQuery("select fieldid from meeting_reminderFields where isOpen = 1 and fieldid = 5");
