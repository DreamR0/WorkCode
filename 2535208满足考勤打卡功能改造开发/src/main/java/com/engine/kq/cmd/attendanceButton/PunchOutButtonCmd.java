...
//    signSection: 2019-03-20 08:30#2019-03-20 18:30
    ...
    //是否开启外勤签到转考勤
    String outsidesign = "";
    //qc2535208判断外勤打卡是否在考勤地点或者满足wifi设置
    String isHave = Util.null2String(params.get("ishave"));
    ...
    kqLog.writeLog(user.getLastname()+"PunchOutButtonCmd>text=" + text+";ismobile="+ismobile);
    ...
    //qc2535208 这里多个判断，如果该地点或wifi位置满足考勤组设置，则把数据插入到考勤表
    if("1".equalsIgnoreCase(outsidesign) || "1".equals(isHave)){
...
//        return ;
      ...
      String punchSql = "insert into HrmScheduleSign(userId,userType,signType,signDate,signTime,clientAddress,isInCom,timeZone,belongdate,signfrom,longitude,latitude,addr,deviceInfo,remark,attachment) "+
          " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
      boolean isok = rs.executeUpdate(punchSql,userId,userType,signType,signDate,signTime,clientAddress,isInCom,
          timeZone,belongdate,signfrom,longitude,latitude,address,deviceInfo,remark,attachment);
