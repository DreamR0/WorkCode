...
public class DocDetailUtil extends BaseBean{
                ...
                if(!rs.next()){
                    ...
                    if(canReader){
                        ...
                        if(rs.next()) {
                            ...
                            //qc2440302 如果该用户不为指定角色，则不允许预览（只针对pdf文件）
                        ...
                        while (docImageManager.next()) {
                            ...
                            new BaseBean().writeLog("==zj==(流程readOnLine)"+readOnLine);
                            docAttr.put("tooLarge",tooLarge ? "1" : "0");
    ...
    public List<String> getAttachViewFileType(Boolean pcView,Boolean isIE){
        ...
        //判断图纸格式是否 支持预览
//        RecordSet rs = new RecordSet();
//        rs.executeQuery("select * from doctypeview where filetype='dwg' and status=1");
//        if(rs.next()){
//            extnames.add("dwg");
//            extnames.add("dxf");
//        }
        List<String> cadList = ImageConvertUtil.cadViewExts();
        extnames.addAll(cadList);

        BaseBean bb = new BaseBean();
        if(Util.null2String(bb.getPropValue("doc_yueliu", "openyueliu"), "-1").equals("1")){
            extnames.add("avi");
            extnames.add("mpg");
            extnames.add("wmv");
            extnames.add("mpeg");
