...
//              1-按加班时长比例转调休时长、2-按加班时长范围设置转调休时长、3-按加班的时间段设置转调休时长
    ...
    public static boolean createData(String ruleId, String belongYear, int organizationType, String organizationIds, String operator, boolean canUpdate) {
        ...
        try {
            ...
            while (recordSet.next()) {
                ...
                if ("41".equals(resourceId)){
                    new BaseBean().writeLog("==zj==(oldBalanceEntity)" + JSON.toJSONString(oldBalanceEntity));
                    new BaseBean().writeLog("==zj==(addOrUpdate)" + JSON.toJSONString(addOrUpdate));
                    new BaseBean().writeLog("==zj==(canUpdate)" + JSON.toJSONString(canUpdate));
                }
    ...
    private static double getBaseAmount(Map<String, Object> params) {
        ...
        try {
            ...
            if ((baseAmount_releaseDate.doubleValue() <= 0 && !isNext) && date4CalcAmount.compareTo(ageLimitChangeDate) < 0) {
                return 0;
            }
             ...
             *              如果【calcMethod】==0？假期基数按比例精确计算
             */
            //ageLimitChangeDate:belongyear+workStartDate.substring(4) | baseAmountReleaseDate:belongYear + "-01-01" baseAmountChangeDate:belongYear + "-01-01"
        ...
        //==z
        if ("42".equals(params.get("resourceId"))){
            new BaseBean().writeLog("==zj==(baseAmount)" + JSON.toJSONString(baseAmount));
        }
