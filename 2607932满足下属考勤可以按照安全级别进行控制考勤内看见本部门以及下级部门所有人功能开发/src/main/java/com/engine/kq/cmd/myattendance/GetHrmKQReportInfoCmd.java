...
public class GetHrmKQReportInfoCmd extends AbstractCommonCommand<Map<String, Object>> {
    ...
    public Map<String, Object> execute(CommandContext commandContext) {
        ...
        try {
            ...
            //这里如果为指定安全值则放开权限
             String departmentLevel = new BaseBean().getPropValue("qc2607932","departmentlevel");    //获取查看分部及下级分部安全级别
            Boolean isHave = false;
            String[] departmentLevels = departmentLevel.split(",");
            String userLevel = Util.null2String(user.getSeclevel());
            for (int i = 0; i < departmentLevels.length; i++) {
                if (userLevel.equals(departmentLevels[i])){
                    isHave = true;
                }
            }
            new BaseBean().writeLog("==zj==(userLevel)" + JSON.toJSONString(userLevel));
            new BaseBean().writeLog("==zj==(isHave)" + JSON.toJSONString(isHave));
            if (!resourceId.equals("" + user.getUID()) && !isIndirectSuperId(resourceId, user.getUID() + "", resourceComInfo) && !HrmUserVarify.checkUserRight("HrmResource:Absense", user, departmentId) && !isHave) {
            ...
            itemMap.put("title", "公出");//公出
            itemMap.put("type", "outdayscount");
            itemMap.put("value", outdayscount);
            itemList.add(itemMap);

            leavebackcount = kcu.getLeaveBackCount(resourceId,fromDate,toDate);//销假
            itemMap = new HashMap<String, Object>();
            itemMap.put("name", "leavebackcount");
            itemMap.put("title", "销假");//公出
            itemMap.put("type", "leavebackcount");
            itemMap.put("value", leavebackcount);
            itemList.add(itemMap);

            leavecount = kcu.getUnusualdata(resourceId,fromDate,toDate,"leave");//请假-私假
            itemMap = new HashMap<String, Object>();
            itemMap.put("name", "leavecount");
            itemMap.put("title", "私假");//私假
