...
import com.api.cusomization.qc2721235.util.KqUtil;
...
//			orderBy = "order by "+orderBy;
			...
			while (rs.next()) {
				...
				while (kqReportFieldComInfo.next()){
					...
					if(fieldName.equals("subcompany")){
					...
					}else if (fieldName.equals("paidLeaveTotal")){
						//==z 调休计算
						fieldValue = Util.null2String(kqUtil.getRestAmount(id,fromDate,toDate));
					}else if (fieldName.equals("nightSerialCount")){
						//==z 夜班次数计算
						fieldValue = Util.null2String(kqUtil.getSerialCount(id,fromDate,toDate));
					}else if (fieldName.equals("restDayOvertime_final")){
						//==z 计算抵扣调休的休息日加班时长
						fieldValue = kqUtil.getRestAmount(id,fromDate,toDate,flowData,3);
					}else if (fieldName.equals("workingDayOvertime_final")){
						//==z 计算抵扣调休的工作日加班时长
						fieldValue = kqUtil.getRestAmount(id,fromDate,toDate,flowData,2);
						...
						for(int i=0;serialIds!=null&&i<serialIds.size();i++){
							data.put(serialIds.get(i), Util.null2String(kqReportBiz.getSerialCount(id,fromDate,toDate,serialIds.get(i))));
						}
