          ...
          if(!leavetypeObject.isEmpty()){
            ...
            for (Entry<String, Object> typeEntry : keave_set) {
              ...
              if(balanceEnable){
                ...
                if( balanceOfLeaveDays <= 0  && !isTiaoxiu ){
                  //可请 带薪假时长为0 不能提交
                  retmap.put("status", "-1");
                  retmap.put("message",  SystemEnv.getHtmlLabelName(390298,user.getLanguage()));
                  return retmap;
                }
                if(leaveDays_Double > balanceOfLeaveDays && !isTiaoxiu){
                  //请假天数大于带薪假时长 不能提交
                  retmap.put("status", "-1");
                ...
                if(isFreezeNodeId(workflowid, nodeid)){
                  ...
                  if(d_duration > 0){
                    ...
                    for(String tmp : allbalanceList){
                      ...
                      if(totalLeavedays > tmp_max_balance && !isTiaoxiu){
                        //请假天数大于带薪假时长 不能提交
                        retmap.put("status", "-1");
                        retmap.put("message",  SystemEnv.getHtmlLabelName(390299,user.getLanguage()));
                        return retmap;
