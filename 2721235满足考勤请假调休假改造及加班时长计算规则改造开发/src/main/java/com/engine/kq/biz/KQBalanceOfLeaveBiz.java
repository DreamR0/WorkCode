...
public class KQBalanceOfLeaveBiz {
     ...
     * @param ruleId
     * @param searchDate
     * @param calcByCurrentDate
     * @param isAll
     * @param isCustom
     * @return
     */
    public static String getRestAmount(String resourceId, String ruleId, String searchDate, boolean calcByCurrentDate, boolean isAll,String fromDate,String toDate,int isCustom) {
        //员工的假期余额
        String balanceOfLeave = "0";
        Map<String, Object> params = new HashMap<>();
        params.put("resourceId",resourceId);
        ...
        try {
            ...
            //如果是调休，获取假期余额的逻辑会不太一样
            if (distributionMode == 5) {
                restAmount = getRestAmountByDis5(ruleId, resourceId, searchDate, calcByCurrentDate, isAll,fromDate,toDate,1);
                return restAmount.setScale(decimalDigit, RoundingMode.HALF_UP).toPlainString();
            }
    ...
    public static BigDecimal getCanUseAmount(String resourceId, String ruleId, String belongYear, BigDecimal baseAmount, String legalOrWelfare, String date) {
        ...
        try {
			...
			//如果人员已离职--根据合同结束日期计算年假天数            
            String resourcestatus = Util.null2String(resourceComInfo.getStatus(resourceId));
            if(resourcestatus.equals("5")){                
                try{
                    String enddate = Util.null2String(resourceComInfo.getEndDate(resourceId));//合同结束日期
                    Calendar calenddate = DateUtil.getCalendar(enddate);
                    //指定日期是那年的第几天
                    dayOfYear = new BigDecimal("" + calenddate.get(Calendar.DAY_OF_YEAR));
                    //指定日期是那年的第几月
                    monthOfYear = new BigDecimal("" + (calenddate.get(Calendar.MONTH) + 1));
                }catch (Exception e){
                    e.printStackTrace();
                }                
            }
...
//              1-按加班时长比例转调休时长、2-按加班时长范围设置转调休时长、3-按加班的时间段设置转调休时长
    ...
    private static Map<String, String> save_paidLeaveEnableDefaultType3(String date,
        ...
        int changeType = KQOvertimeRulesBiz.getChangeType(resourceId, date);//获取当天类型
    ...
    /*插入调休时长*/
        ...
        String sql = "insert into kq_balanceOfLeave(leaveRulesId,resourceId,belongYear,belongMonth,baseAmount,tiaoxiuamount,usedAmount,baseAmount2,extraAmount2,usedAmount2,status,expirationDate,effectiveDate,overtimeType,isDelete,changeType) " +
                "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        flag = recordSet.executeUpdate(sql, leaveRulesId, resourceId, _belongYear, _belongMonth, 0, extraAmount, 0, 0, 0, 0, 0, expirationDate, date, fromCardOrFlow.equals("0") ? "3" : "4", 0,changeType);
        if (!flag) {
    ...
    private static Map<String,String> save_paidLeaveEnableDefaultType1(
        ...
        BigDecimal Deduction = durationOfLeave;
        String DeductionAmount = "";
        //==z 这里查询下是否有负数调休，做下冲抵
        RecordSet rs = new RecordSet();
        String negativeSql = "select * from kq_balanceOfLeave where resourceid='"+resourceId+"' and belongmonth is null and isdelete is null and tiaoxiuamount=.00000";
        recordSet.executeQuery(negativeSql);
        while (recordSet.next()){
            String id = Util.null2String(recordSet.getString("id"));//负数流程id
            BigDecimal negative =new BigDecimal(Util.null2String(recordSet.getString("usedAmount"))) ;//当前负数时长
            //剩余时长 = 加班时长-负数时长
            Deduction = Deduction.subtract(negative);

            if (Deduction.doubleValue() <= 0){
                //说明当前加班时长已经扣完，或者不足以抵扣
                String negativeNewAmount = "";
                BigDecimal negativeNew = BigDecimal.ZERO.subtract(Deduction);//加班时长抵扣后剩余的负数时长
                negativeNewAmount = negativeNew.setScale(5, RoundingMode.HALF_UP).toPlainString();
                String upDateSql = "update kq_balanceOfLeave set usedAmount='"+negativeNewAmount+"'  where resourceid='"+resourceId+"' and id="+id;
                rs.executeUpdate(upDateSql);
                Deduction = BigDecimal.ZERO;
                break;
            }else if (Deduction.doubleValue() > 0){
                //删除该负数流程
                String upDateSql = "delete from kq_balanceOfLeave where resourceid='"+resourceId+"' and id="+id;
                recordSet.executeUpdate(upDateSql);
            }
        }

        //已使用 = 生成加班时长-剩余加班时长
        new BaseBean().writeLog("==zj==(加班生成冲抵)" + durationOfLeave + " | "+ Deduction);
        Deduction  = durationOfLeave.subtract(Deduction);
        DeductionAmount = Deduction.setScale(5, RoundingMode.HALF_UP).toPlainString();
    ...
    /*插入调休时长*/
        ...
        String sql = "insert into kq_balanceOfLeave(leaveRulesId,resourceId,belongYear,belongMonth,baseAmount,tiaoxiuamount,usedAmount,baseAmount2,extraAmount2,usedAmount2,status,expirationDate,effectiveDate,overtimeType,isDelete,changeType) " +
                "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        flag = recordSet.executeUpdate(sql, leaveRulesId, resourceId, _belongYear, _belongMonth, 0, extraAmount, DeductionAmount, 0, 0, 0, 0, expirationDate, date, fromCardOrFlow.equals("0") ? "3" : "4", 0,changeType);
        if (!flag) {
            resultMap.put("status", "-2");
            return resultMap;
        }
    ...
    public static void addUsedAmount(String resourceId, String date, String ruleId, String duration, String type, String requestId, String fromDateDb) {
        try {
            ...
            if (distributionMode == 5) {
                if (recordSet.getDBType().equalsIgnoreCase("sqlserver")
                    ...
                    sql = " select * from kq_balanceOfLeave " +
                            " where (isDelete is null or isDelete<>1) and resourceId=" + resourceId + " and leaveRulesId=" + ruleId + " and (expirationDate is null or expirationDate='' or expirationDate>='" + fromDateDb + "') " +
                            " order by belongYear asc,expirationDate asc,case when changetype=3 then 1 when changetype=2 then 2 else 3 end  ";
                } else {
                    sql = " select * from kq_balanceOfLeave " +
                            " where (isDelete is null or isDelete<>1) and resourceId=" + resourceId + " and leaveRulesId=" + ruleId + " and (expirationDate is null or expirationDate>='" + fromDateDb + "') " +
                            " order by belongYear asc,expirationDate asc,case when changetype=3 then 1 when changetype=2 then 2 else 3 end  ";
                }
            }
            recordSet.executeQuery(sql);
            ...
            int index = 0;
            int indexStatus = 1;
            new BaseBean().writeLog("==zj==(使用余额sql)" + JSON.toJSONString(sql));
            while (recordSet.next()) {
                index++;
                //
                String id = recordSet.getString("id");
                ...
                if (distributionMode == 6) {//如果年假为混合模式（法定年假+福利年假）
                    ...
                    new BaseBean().writeLog("==zj==(调休扣减)" + "resourceId=" + resourceId+",date=" + date+",extraAmount=" + extraAmount.doubleValue()+",usedAmount=" + usedAmount.doubleValue()+
                            "tiaoxiuAmount=" + tiaoxiuAmount+",_duration=" + _duration );
                    /*该假期剩余假期余额不足以扣减，记录错误日志，退出方法*/
                    if (index == total && temp.doubleValue() < 0) {
                        logger.info("该人员的该假期所有的剩余假期余额都不足以扣减。" +
                                "resourceId=" + resourceId + ",date=" + date + ",ruleId=" + ruleId + ",type=" + type + ",duration=" + duration + ",requestId=" + requestId +
                                ",baseAmount=" + baseAmount.doubleValue() + ",extraAmount=" + extraAmount.doubleValue() + ",usedAmount=" + usedAmount.doubleValue());
                        //=z _duration是否大于usedAmount
                        String newUsedAmount = baseAmount.add(tiaoxiuAmount).add(extraAmount).setScale(5, RoundingMode.HALF_UP).toPlainString();
                        _duration = _duration.subtract(baseAmount.add(extraAmount).add(tiaoxiuAmount).subtract(usedAmount));
                        String updateSql = "update kq_balanceOfLeave set usedAmount=" + (newUsedAmount) + " where id=" + id;
                        updateList.add(updateSql);
                    ...
                    if (temp.doubleValue() >= 0) {
                        String newUsedAmount = usedAmount.add(_duration).setScale(5, RoundingMode.HALF_UP).toPlainString();
                        //==z
                        _duration = _duration.subtract(_duration);
                        String updateSql = "update kq_balanceOfLeave set usedAmount=" + (newUsedAmount) + " where id=" + id;
                        updateList.add(updateSql);
            ...
            new BaseBean().writeLog("==zj==(调休余额)" +resourceId+ " | " +  JSON.toJSONString(_duration));
            if (index == 0 || indexStatus==0 || _duration.compareTo(BigDecimal.ZERO) > 0) {
                //所属年份
                String belongYear = fromDateDb.substring(0, 4);
                //基数
                BigDecimal baseAmount = new BigDecimal("0");
                //额外
                BigDecimal extraAmount = new BigDecimal("0");
                //福利年假
                BigDecimal baseAmount2 = new BigDecimal("0");
                //福利额外
                BigDecimal extraAmount2 = new BigDecimal("0");
                //加班生成调休
                BigDecimal tiaoxiuAmount = new BigDecimal("0");
                //已休
                BigDecimal usedAmount = new BigDecimal("0");
                //福利已修
                BigDecimal usedAmount2 = new BigDecimal("0");
                ...
                usageHistoryEntity.setOldUsedAmount2("0");
                usageHistoryEntity.setOldMinimumUnit("" + minimumUnit);
                usageHistoryEntity.setNewMinimumUnit("" + minimumUnit);



                BigDecimal temp = new BigDecimal("0").subtract(_duration);
                String updateSql = "";
                /*该假期剩余假期余额不足以扣减，记录错误日志，退出方法*/
                //==z 这里修改下
                if (temp.doubleValue() < 0) {
                    logger.info("该人员的该假期所有的剩余假期余额都不足以扣减。" +
                            ...
                            ",baseAmount=" + baseAmount.doubleValue() + ",extraAmount=" + extraAmount.doubleValue() + ",usedAmount=" + usedAmount.doubleValue() +
                            ",baseAmount2=" + baseAmount2.doubleValue() + ",extraAmount2=" + extraAmount2.doubleValue() + ",usedAmount2=" + usedAmount2.doubleValue()+
                            "tiaoxiuAmount="+ tiaoxiuAmount.doubleValue());
                    String newUsedAmount = usedAmount.add(_duration).setScale(5, RoundingMode.HALF_UP).toPlainString();
                    if (isTiaoxiu) {//调休
                        updateSql = "INSERT INTO KQ_BalanceOfLeave(resourceId,leaveRulesId,belongYear,overtimeType,baseAmount,extraAmount,usedAmount,effectiveDate) " +
                                "VALUES(" + resourceId + "," + ruleId + ",'" + belongYear + "','4',0,0," + newUsedAmount + ",'" + fromDateDb + "')";
                        usageHistoryEntity.setNewUsedAmount(newUsedAmount);
...
//第二个孩子的情况
    ...
    private static BigDecimal getRestAmountByDis5(String ruleId, String resourceId, String searchDate, boolean calcByCurrentDate, boolean isAll) {
        ...
        try {
            ...
            new BaseBean().writeLog("==zj==(系统标准sql)" + JSON.toJSONString(sql));
            recordSet.executeQuery(sql);
            while (recordSet.next()) {
     ...
     * @return
     */
    private static BigDecimal getRestAmountByDis5(String ruleId, String resourceId, String searchDate, boolean calcByCurrentDate, boolean isAll,String fromDate,String toDate,int isCustom) {
        BigDecimal restAmount = new BigDecimal("0");
        try {
            //获取当前日期
            Calendar today = Calendar.getInstance();
            ...
            if (recordSet.getDBType().equalsIgnoreCase("sqlserver")
                    || recordSet.getDBType().equalsIgnoreCase("mysql")) {
                sql = " select sum(tiaoxiuamount) as allTiaoxiuamount,sum(baseAmount) as allBaseAmount,sum(extraAmount) as allExtraAmount,sum(usedAmount) as allUsedAmount from KQ_BalanceOfLeave " +
                        " where (isDelete is null or isDelete<>1) and resourceId=" + resourceId + " and leaveRulesId=" + ruleId;
            } else {
                sql = " select sum(tiaoxiuamount) as allTiaoxiuamount,sum(baseAmount) as allBaseAmount,sum(extraAmount) as allExtraAmount,sum(usedAmount) as allUsedAmount from KQ_BalanceOfLeave " +
                        " where (isDelete is null or isDelete<>1) and resourceId=" + resourceId + " and leaveRulesId=" + ruleId ;
            }
            ...
            new BaseBean().writeLog("==zj==(考勤报表sql)" + JSON.toJSONString(sql));
            recordSet.executeQuery(sql);
            while (recordSet.next()) {
                BigDecimal _baseAmount = new BigDecimal(Util.null2s(recordSet.getString("allBaseAmount"), "0"));
                BigDecimal _extraAmount = new BigDecimal(Util.null2s(recordSet.getString("allExtraAmount"), "0"));
