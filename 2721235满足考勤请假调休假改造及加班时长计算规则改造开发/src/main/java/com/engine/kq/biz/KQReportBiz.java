...
//    DecimalFormatSymbols symbols = new DecimalFormatSymbols();
  ...
  public Map<String, String> realDate(String fromDate, String toDate, User user, String reportType) {
    ...
    if("-100".equals(reportType)) {
      RecordSet rs = new RecordSet();
      String departmentId = "";
      rs.executeQuery("select subcompanyid1, departmentId from hrmresource where id = ?", user.getUID());
      if (rs.next()) {
        departmentId = StringUtil.vString(rs.getString("departmentId"));
      }
      hasRight = HrmUserVarify.checkUserRight("MobileSignInfo:Manage", user, departmentId);
    } else {
      hasRight = new KQReportBiz().hasReportRight(reportType,""+user.getUID());
    }
