...
import com.customization.qc2215400.DateUtil;
	...
	public Map<String, Object> execute(CommandContext commandContext) {
      ...
      if(kqWorkTimeEntity != null){
        ...
        if("3".equalsIgnoreCase(kqType)){
            ...
            try{
                 overTimes = du.TimesTampChange(fromDate, fromTime, toDate, toTime,resourceId);
                 new BaseBean().writeLog("==zj==(可以转换成功吗)" + overTimes);

            }catch (Exception e){
            }

          writeLog("自由班制单独计算加班时长");
          retmap.put("status", "1");
          retmap.put("duration",overTimes);
          retmap.put("message",  ""+ SystemEnv.getHtmlLabelName(10005330,weaver.general.ThreadVarLanguage.getLang())+"");
          return retmap;
        }
      }
