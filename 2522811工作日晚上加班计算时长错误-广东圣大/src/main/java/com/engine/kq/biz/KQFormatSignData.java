...
public class KQFormatSignData extends BaseBean {
  ...
  public List<Object> getSignInfo(String userId, TimeScopeEntity signTimeScope,
    ...
    try {
      ...
      List<TimeScopeEntity> signTime1 = workTime.getSignTime();

      String endTime = "";//上午班次的下班时间
      if (workTime1.size() > 0) {
        TimeScopeEntity timeScopeEntity = workTime1.get(0);
      ...
      if (shiftCount == 2 && shiftI == 0) {
        ...
        //如果午间存在签退抵扣打卡，上午不需要取卡
        boolean has_flow_deduct_card = false;
        Map<String, String> flow_deduct_card_map_noon = getflowDeductCardSql(userId, kqDate, "", endTime);
        if (flow_deduct_card_map_noon.containsKey("signoff")) {
          String deduct_signofftime = Util.null2String(flow_deduct_card_map_noon.get("signoff"));
          if (deduct_signofftime.length() > 0) {
            has_flow_deduct_card = true;
          }
        }
        ...
        rs.executeQuery(sql);
        if (rs.next() && !has_flow_deduct_card) {
          String signId = Util.null2String(rs.getString("id"));
          String signDate = Util.null2String(rs.getString("signdate"));
          String signTime = Util.null2String(rs.getString("signtime"));
          String signDateTime = signDate + " " + signTime;
        ...
        if(has_flow_deduct_card){
          for (int j = 0; lsCheckInfo != null && j < lsCheckInfo.size(); j++) {
            ...
            if (checkInfoInner.get("signType").equals("2")) {//签退
              ...
              String deduct_signofftime = Util.null2String(checkInfoInner.get("deduct_signofftime"));
              if (!"".equals(signTimeInner)) {
                checkInfoInner.remove("signId");
                checkInfoInner.put("signType", "2");
                checkInfoInner.put("signDate", signDateInner);//签到签退日期
                checkInfoInner.put("signTime", "");//签到签退时间
                checkInfoInner.put("deduct_signofftime", deduct_signofftime);//流程抵扣作为打卡时间
                checkInfoInner.put("signStatus", ButtonStatusEnum.NORMAL.getStatusCode());
        ...
        sql += " and " + noonSignTimeSql + " order by signdate, signtime";
        rs.executeQuery(sql);
        new BaseBean().writeLog("==zj==(下午取卡sql)" + JSON.toJSONString(sql));
        ...
        if (flow_deduct_card_map_noon.containsKey("signoff")) {
          ...
          if (deduct_signofftime.length() > 0) {
            ...
            int counts = rs.getCounts();
            if(counts==1){
              if (lsCheckInfo.size() > 0) {
                lsCheckInfo.remove(0);
              }
            }
          }
        }else {
        ...
        //qc2522811下班卡取离下班之后最近的
        SignCardUtil signCardUtil = new SignCardUtil();
        int changeType = KQOvertimeRulesBiz.getChangeType(userId, kqDate);
        Boolean isNight = signCardUtil.getserialName(userId, kqDate);
        if ( changeType == 2 && !isNight){
            //将下午下班卡设置为距离下班时间最近的
            signCardUtil.getOutCard(userId,signEndDateTime4Afternoon,lsCheckInfo);
        }
        if (!relatedFlag) {
