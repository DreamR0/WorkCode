...
public class KQFlowDataBiz {
  ...
  public List<SplitBean> getOverTimeData(Map<String,Object> flowMaps){
    ...
    new BaseBean().writeLog("==zj==(leaveSql1)" + JSON.toJSONString(leaveSql));
    rs.execute(leaveSql);
    while(rs.next()){
      SplitBean splitBean = new SplitBean();
      ...
      String serial_type = rs.getString("serial_type");
      new BaseBean().writeLog("==zj==(serial_type)" + JSON.toJSONString(serial_type));
