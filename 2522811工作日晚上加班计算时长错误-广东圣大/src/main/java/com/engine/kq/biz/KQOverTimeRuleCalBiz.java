...
import com.api.cusomization.kq.OverTimeUtil;
import com.api.cusomization.kq.SignCardUtil;
...
public class KQOverTimeRuleCalBiz {
    ...
    private String card_from0 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "from0"), "04:00");
    private String card_to0 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "to0"), "12:59");
    private String card_from1 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "from1"), "13:00");
    private String card_to1 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "to1"), "17:29");
    private String card_from2 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "from2"), "17:30");
    private String card_to2 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "to2"), "03:59");
    private boolean splitPointUsed = false;
...
//        需审批，以打卡为准，取流程和打卡的交集
  ...
  public void updateTiaoXiu(String resourceid, String fromDate, String toDate){
    ...
    private void doComputingMode4(String resourceid, String splitDate,
        ...
        //add
        String restpointtime = "";
        String lastworkday = "";
        int count = 1;
        //获取加班流程数据
        Map<String, List<SplitBean>> splitBeanMaps1 = Maps.newHashMap();
        getOverTimeFlowData(resourceid, splitDate, splitDate, splitBeanMaps1, dateFormatter);

        //==zj 多个判断，休息日夜班走标准
        String serial_type = "";
        List<SplitBean> flowBeans = splitBeanMaps1.get(resourceid + "_" + splitDate);
        new BaseBean().writeLog("==zj==(新加班流程)" + JSON.toJSONString(flowBeans));
        for (int i = 0; i < flowBeans.size(); i++) {
            SplitBean bean = flowBeans.get(i);
            serial_type  = bean.getSerial_type();
        }
        new BaseBean().writeLog("==zj==(当前加班流程是不是休息日夜班)" + JSON.toJSONString(serial_type));

        Map<String, Object> restpointmap = getrestpoint(resourceid, splitDate, dateFormatter,serial_type);
        if (null != restpointmap && !restpointmap.isEmpty()) {
            //表示为休息日或者节假日
            count = Util.getIntValue(Util.null2String(restpointmap.get("count")), 1);
        }
        for (int index = 0; index < count; index++) {
            restpointtime = Util.null2String(restpointmap.get("restpoint" + index));
            lastworkday = Util.null2String(restpointmap.get("lastworkday"));
            //end
            ...
            //获取加班打卡数据
            getOverTimeCardDataMap(resourceid, splitDate, splitDate, dateFormatter, kqTimesArrayComInfo, overRulesDetailMap, changeTypeMap, lsCheckInfoMaps, eventMap, restpointmap, index);
            ...
            while (iterator.hasNext()) {
                ...
                if (kqOvertimeCardBean != null) {
                    ...
                    Map<String, String> signinoffMap = buildOvertimeCard(kqOvertimeCardBean, resourceid, realSplitDate, kqTimesArrayComInfo, restTimeMap, changeType_key, initArrays, hasOverTime4SignList,
                            overRulesDetailMap, true, overtimeLogMap, restpointmap);
...
//        return;
            ...
            while (iterator.hasNext()) {
                ...
                if (kqOvertimeCardBean != null) {
                    ...
                    Map<String, String> signinoffMap = buildOvertimeCard(kqOvertimeCardBean, resourceid, realSplitDate, kqTimesArrayComInfo, restTimeMap, changeType_key, initArrays, Lists.newArrayList(), overRulesDetailMap,
                            true, overtimeLogMap, restpointmap);
                    logOvertimeMap(overtimeLogMap, signinoffMap, "获取上下班打卡数据|signinoffMap");
                    bb.writeLog("1031 ===> signinoffMap = "+signinoffMap);
                    bb.writeLog("1031 ===> kqOvertimeCardBean = "+kqOvertimeCardBean);
                    String signinTime = Util.null2String(signinoffMap.get("signinTime"));
                    String signoutTime = Util.null2String(signinoffMap.get("signoutTime"));
                    ...
                    //--qc2522811
                    SignCardUtil signCardUtil = new SignCardUtil();
                    OverTimeUtil overTimeUtil = new OverTimeUtil();
                    //--

                    new BaseBean().writeLog("==zj==(打卡情况--纯打卡)" +"人员id:"+resourceid+ " -- 签到:"+signinTimeIndex+" -- 当天类型:"+count+" -- 签出:"+signoutTimeIndex+" -- 考勤日期"+signinDate );
                    if (signinTimeIndex < signoutTimeIndex) {
                        int over_count = 0;
                        //qc2522811这里对工作日且非夜班班次进行判断
                        Boolean isNight = signCardUtil.getserialName(resourceid, splitDate);
                        Boolean isLeave = overTimeUtil.isLeave(resourceid,splitDate,serial_type);
                        new BaseBean().writeLog("==zj==(当天是否有加班情况--纯打卡)" +"人员id:"+resourceid+ " -- 是否有请假流程:"+isLeave+" -- 当天类型:"+count+" -- 是否夜班:"+isNight+" -- 考勤日期"+signinDate );
                        if (count == 1 && !isNight){
                            //qc2522811先获取当天下班是否满足三次打卡
                            Boolean isThreeCard = overTimeUtil.isThreeCard(resourceid, signinDate);
                            if (!isThreeCard && !isLeave){
                                //qc2522811不满足三次打卡加班时长为0
                                over_count = 0;
                            }else {
                                if (isLeave /*&& !isThreeCard*/){
                                    //如果下班时间在请假范围，且没有三次卡，去下班第一张卡
                                    overTimeUtil.overTimeBeginCardLeave(resourceid,signinDate,initArrays);
                                }else {
                                    //如果有3次卡就去下班第二张卡
                                    overTimeUtil.overTimeBeginCard(resourceid,signinDate,initArrays);
                                }
                                over_count = kqTimesArrayComInfo.getCnt(initArrays, signinTimeIndex, signoutTimeIndex, 1);
                                new BaseBean().writeLog("==zj==(纯打卡overcount)" + JSON.toJSONString(over_count));
                            }
                        }else {
                              //根据班次区别打卡有效范围
                              String overCountStartTime = "";
                              String overCountEndTime = "";
                              int overCountStartTimeIndex = 0;
                              int overCountEndTimeIndex = 0;


                              if (count == 3){
                                  if (index == 0){
                                      overCountStartTime = Util.null2String(restpointmap.get("from0"));
                                      overCountEndTime = Util.null2String(restpointmap.get("to0"));
                                      overCountStartTimeIndex = kqTimesArrayComInfo.getArrayindexByTimes(overCountStartTime);
                                      overCountEndTimeIndex = kqTimesArrayComInfo.getArrayindexByTimes(overCountEndTime);
                                  }
                                  if (index == 1){
                                      overCountStartTime = Util.null2String(restpointmap.get("from1"));
                                      overCountEndTime = Util.null2String(restpointmap.get("splitPoint"));
                                      overCountStartTimeIndex = kqTimesArrayComInfo.getArrayindexByTimes(overCountStartTime);
                                      overCountEndTimeIndex = kqTimesArrayComInfo.getArrayindexByTimes(overCountEndTime);
                                  }
                                  if (index == 2){
                                      overCountStartTime = Util.null2String(restpointmap.get("splitPoint"));
                                      overCountEndTime = Util.null2String(restpointmap.get("to2"));
                                      overCountEndTime = kqTimesArrayComInfo.turn24to48Time(overCountEndTime);
                                      overCountStartTimeIndex = kqTimesArrayComInfo.getArrayindexByTimes(overCountStartTime);
                                      overCountEndTimeIndex = kqTimesArrayComInfo.getArrayindexByTimes(overCountEndTime);
                                  }
                                  bb.writeLog("1031 ==>overCountStartTime = "+overCountStartTime);
                                  bb.writeLog("1031 ==>overCountEndTime = "+overCountEndTime);
                                  over_count = kqTimesArrayComInfo.getCnt(initArrays, overCountStartTimeIndex, overCountEndTimeIndex, 1);
                              }else {
                                  over_count = kqTimesArrayComInfo.getCnt(initArrays, signinTimeIndex, signoutTimeIndex, 1);
                              }
                        }
                        bb.writeLog("10313 ==>over_count = "+over_count);
    ...
    private void doComputingMode2(String resourceid, String splitDate,
        ...
        Map<String, Object> restpointmap = getrestpoint(resourceid, splitDate, dateFormatter,serial_type);
        new BaseBean().writeLog("==zj==(restpointmap是否)" + JSON.toJSONString(restpointmap));
        if (null != restpointmap && !restpointmap.isEmpty() ) {
            //表示为休息日或者节假日
            count = Util.getIntValue(Util.null2String(restpointmap.get("count")), 1);
        }
        new BaseBean().writeLog("==zj==(count)" + JSON.toJSONString(count));
        for (int index = 0; index < count; index++) {
            restpointtime = Util.null2String(restpointmap.get("restpoint" + index));
            lastworkday = Util.null2String(restpointmap.get("lastworkday"));
            //end
      ...
      kqLog.info("count:"+count+":index:"+ index+":restpointmap:"+ JSONObject.toJSONString(restpointmap));
     kqLog.info("lsCheckInfoMaps:"+JSON.toJSON(lsCheckInfoMaps));
...
//                生成过加班，但是其实只用了1小时，还需要拿这个加班流程来，减去已经用的1小时，d_mins就是这个加班流程实际可用的加班流程时长
                            ...
                            if (d_mins > 0) {
                                ...
                                Map<String, String> signinoffMap = buildOvertimeCard(kqOvertimeCardBean, resourceid, realSplitDate, kqTimesArrayComInfo, restTimeMap, changeType_key, initArrays, hasOverTimeList,
                                        overRulesDetailMap, false, overtimeLogMap, restpointmap);
                                ...
                                if (signinTimeIndex < signoutTimeIndex) {
                                    ...
                                    int overCountStartTimeIndex = 0;
                                    int overCountEndTimeIndex = 0;
                                    //这里多个判断，如果是非工作日且夜班走标准
                                     serial_type = splitBean.getSerial_type();
                                    new BaseBean().writeLog("==zj==(流程打卡判断)" + splitDate + " | " + count + " | " + serial_type);
                                    if (count == 3 && !"1".equals(serial_type)) {
                                        if (index == 0){
                                            overCountStartTime = Util.null2String(restpointmap.get("from0"));
                                            overCountEndTime = Util.null2String(restpointmap.get("to0"));
                                            overCountStartTimeIndex = kqTimesArrayComInfo.getArrayindexByTimes(overCountStartTime);
                                        ...
                                        bb.writeLog("1115 ==>overCountStartTime2 = "+overCountStartTime);
                                        bb.writeLog("1115 ==>overCountEndTime2 = "+overCountEndTime);
                                        over_count = kqTimesArrayComInfo.getCnt(initArrays, overCountStartTimeIndex, overCountEndTimeIndex, 1);//休息日
                                        new BaseBean().writeLog("==zj==(休息日非夜班)" + JSON.toJSONString(over_count));
                                    }else {
                                        //qc2522811这里对工作日且非夜班班次进行判断
                                        SignCardUtil signCardUtil = new SignCardUtil();
                                        OverTimeUtil overTimeUtil = new OverTimeUtil();
                                        Boolean isNight = signCardUtil.getserialName(resourceid, splitDate);
                                        Boolean isLeave = overTimeUtil.isLeave(resourceid,signinDate,serial_type);
                                        new BaseBean().writeLog("==zj==(isLeave2)" + JSON.toJSONString(isLeave));
                                        if (count == 1 && !isNight && !"1".equals(serial_type)){
                                            ...
                                            if (!isThreeCard && !isLeave){
                                                ...
                                                if (isLeave /*&& !isThreeCard*/){
                                                    //没有满足三次打卡，但是有请假流程取，下班之后的第一张卡
                                                    overTimeUtil.overTimeBeginCardLeave(resourceid,signinDate,initArrays);
                                                }else {
                                                    //满足三次打卡，就去下班之后的第二张卡
                                                     overTimeBeginCardIndex = overTimeUtil.overTimeBeginCard(resourceid, signinDate, initArrays);
                                                    new BaseBean().writeLog("==zj==(overTimeBeginCardIndex)" + JSON.toJSONString(overTimeBeginCardIndex));
                                                }
                                                over_count = kqTimesArrayComInfo.getCnt(initArrays, overTimeBeginCardIndex, signoutTimeIndex, 1);
                                                new BaseBean().writeLog("==zj==(over_count2)" + JSON.toJSONString(over_count));
                                            }
                                        }else {
                                            //夜班班次走标准
                                            new BaseBean().writeLog("==zj==(休息日夜班走标准)");
                                            new BaseBean().writeLog("==zj==(休息日夜班)" + signinTimeIndex +  " | " + signoutTimeIndex);
                                            over_count = kqTimesArrayComInfo.getCnt(initArrays, signinTimeIndex, signoutTimeIndex, 1);
                                            new BaseBean().writeLog("==zj==(休息日夜班over_count)" + JSON.toJSONString(over_count));
                                        }
...
//                    }
    ...
    public Map<String, String> buildOvertimeCard(KQOvertimeCardBean kqOvertimeCardBean,
        ...
        return buildOvertimeCard(kqOvertimeCardBean, resourceid, splitDate, kqTimesArrayComInfo, restTimeMap, changeType_key, initArrays, hasOverTimeList,
                overRulesDetailMap, needHasOverTime, overtimeLogMap, null);
    }
    ...
    public Map<String, String> buildOvertimeCard(KQOvertimeCardBean kqOvertimeCardBean,
                                                 ...
                                                 List<Map<String, String>> hasOverTimeList,
                                                 Map<String, KQOvertimeRulesDetailEntity> overRulesDetailMap, boolean needHasOverTime,
                                                 Map<String, Object> overtimeLogMap, Map<String, Object> restpointmap) throws Exception {
        ...
        //add
        if (null != restpointmap && !restpointmap.isEmpty()) {//根据上个工作日的上班情况只计算工作时间段段的加班情况，非工作时间都去掉
            List<Object> result = (List<Object>) restpointmap.get("needpassTimeScope");
            bb.writeLog("1031 result ==> "+ result);
            for (int i = 0; i < result.size(); i++) {
                Map<String, Object> needpass = (Map<String, Object>) result.get(i);
                int from = Util.getIntValue(Util.null2String(needpass.get("from")), 0);
                int to = Util.getIntValue(Util.null2String(needpass.get("to")), 0);
                if (from < to) {
                    Arrays.fill(initArrays, from, to, -2);
                }
            }

            int tmpfrom = kqTimesArrayComInfo.getArrayindexByTimes(card_from0);
            int tmpto = kqTimesArrayComInfo.getArrayindexByTimes("08:00");

            if (tmpfrom < tmpto) {
                Arrays.fill(initArrays, tmpfrom, tmpto, -2);
            }
        }
        //end
    ...
    public void getOverTimeCardDataMap(String resourceid, String fromDate, String toDate,
        ...
        getOverTimeCardDataMap(resourceid, fromDate, toDate, dateFormatter, kqTimesArrayComInfo, overRulesDetailMap, changeTypeMap, lsCheckInfoMaps, eventMap, null, 0);
    }
    ...
    public void getOverTimeCardDataMap(String resourceid, String fromDate, String toDate,
                                       ...
                                       Map<String, KQOvertimeRulesDetailEntity> overRulesDetailMap,
                                       Map<String, Integer> changeTypeMap, Map<String, KQOvertimeCardBean> lsCheckInfoMaps,
                                       Map<String, Object> eventMap, Map<String, Object> restpointmap, int index) throws Exception {

        KQOverTimeFlowBiz kqOverTimeFlowBiz = new KQOverTimeFlowBiz();

        List<Object> lsCheckInfos = Lists.newArrayList();
        Map<String, String> result = getSignData(resourceid, fromDate, dateFormatter, eventMap, lsCheckInfos, changeTypeMap, overRulesDetailMap, restpointmap, index);
        ...
        if (!lsCheckInfos.isEmpty()) {
            ...
            if (pre_has_cut_point == 0) {
                ...
                getSignData(resourceid, preSplitDate, dateFormatter, eventMap, pre_lsCheckInfos, changeTypeMap, overRulesDetailMap, restpointmap, index);
    ...
    public Map<String, Object> getrestpoint(String resourceid, String belongDate, DateTimeFormatter dateFormatter,String serial_type) {
        System.out.println("lzr getrestpoint");
        Map<String, Object> restmap = Maps.newHashMap();
        if (null == dateFormatter) {
            dateFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        }
        KQWorkTime kqWorkTime = new KQWorkTime();
        WorkTimeEntity curWorkTime = kqWorkTime.getWorkTime(resourceid, belongDate);
        //多个判断如果是也夜班走标准
        if (curWorkTime == null || curWorkTime.getWorkMins() == 0 && !"1".equals(serial_type)) {//当前是休息日或者节假日
            System.out.println("当前是休息日或者节假日");
            KQTimesArrayComInfo kqTimesArrayComInfo = new KQTimesArrayComInfo();
            ...
            Map<String, String> overtimemap = Maps.newHashMap();
            //qc2613259 从配置文件获取时间段
            KQGroupMemberComInfo kqGroupMemberComInfo = new KQGroupMemberComInfo();
            KQGroupEntity kQGroupEntity = kqGroupMemberComInfo.getUserKQGroupInfo(resourceid,belongDate,true);
            String groupId = kQGroupEntity.getId();
            String ruleNum = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "ruleNum"), "1");
            String activeRule = "1";
            bb.writeLog("10312 ==> ruleNum = "+ruleNum);
            for (int i = 1; i <= Integer.parseInt(ruleNum); i++) {
                String key = "rule"+i;
                bb.writeLog("10312 ==> key = "+key);
                String groupIds = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", key));
                List<String> idList = Arrays.asList(groupIds.split(","));
                if (idList.contains(groupId)){
                    bb.writeLog("10315 ==> active i = "+i);
                    activeRule = i+"";
                    break;
                }
            }
            bb.writeLog("10312 ==> user = "+resourceid);
            bb.writeLog("10312 ==> groupId = "+groupId);
            bb.writeLog("10312 ==> activeRule = "+activeRule);
            String from0 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "from0_"+activeRule));
            card_from0 = from0;
            String to0 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "to0_"+activeRule));
            String from1 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "from1_"+activeRule));
            String to1 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "to1_"+activeRule));
            String from2 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "from2_"+activeRule));
            String to2 = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "to2_"+activeRule));
            String splitPoint = Util.null2String(Prop.getInstance().getPropValue("overtime4rest", "splitPoint_"+activeRule));

            overtimemap.put("from0", from0);
            overtimemap.put("to0", to0);
            overtimemap.put("from1", from1);
            overtimemap.put("to1", to1);
            overtimemap.put("from2", from2);
            overtimemap.put("to2", to2);


            List<Object> needpassTimeScope = Lists.newArrayList();
            Map<String, Integer> tmp = Maps.newHashMap();
            for (int i = 0; i < 3; i++) {
                String key = "restpoint" + i;
//        String from = Util.null2String(Prop.getInstance().getPropValue("overtime4rest","from"+i));
//        String to = Util.null2String(Prop.getInstance().getPropValue("overtime4rest","to"+i));
                int workBeginIdx = kqTimesArrayComInfo.getArrayindexByTimes(overtimemap.get("from" + i));
                int workEndIdx = kqTimesArrayComInfo.getArrayindexByTimes(overtimemap.get("to" + i));
                ...
                //假设是一天三次上下班【08:00-12:00,13:00-18:00,19:00-20:00】
                if (tmp.isEmpty()) {//第一次
                    tmp.put("from", workBeginIdx);//08:00
                    tmp.put("to", workEndIdx);//12:00
                } else {//第二次或第三次
                    int from = tmp.get("from");//08:00
                    int to = tmp.get("to");//12:00
                    if (to < workBeginIdx) {
                        Map<String, Object> needpass = Maps.newHashMap();
                        needpass.put("from", to);//12:00
                        needpass.put("to", workBeginIdx);//13:00
                        needpassTimeScope.add(needpass);
                    }

                    tmp.put("from", workBeginIdx);//13:00
                    tmp.put("to", workEndIdx);//18:00
                }
                restmap.put(key, restpoint.length() == 5 ? restpoint + ":00" : restpoint);
            }
            restmap.put("count", 3);
            restmap.put("needpassTimeScope", needpassTimeScope);
            restmap.put("from0", from0);
            restmap.put("to0", to0);
            restmap.put("from1", from1);
            restmap.put("to1", to1);
            restmap.put("from2", from2);
            restmap.put("to2", to2);
            restmap.put("splitPoint", splitPoint);
        ...
        bb.writeLog("1031 ===> restmap ==> " +restmap );
        return restmap;
    }
    ...
    public Map<String, String> getSignData(String resourceid, String belongDate,
        ...
        return getSignData(resourceid, belongDate, dateFormatter, eventMap, lsCheckInfos, changeTypeMap, overRulesDetailMap, null, 0);
    }

    public Map<String, String> getSignData(String resourceid, String belongDate,
                                           DateTimeFormatter dateFormatter, Map<String, Object> eventMap,
                                           List<Object> lsCheckInfos, Map<String, Integer> changeTypeMap,
                                           Map<String, KQOvertimeRulesDetailEntity> overRulesDetailMap, Map<String, Object> restpointmap, int index) throws Exception {
        ...
        if (workTime == null || workTime.getWorkMins() == 0) {
            Map<String, Object> signMap = getRestSignInfo(resourceid, splitDate, preDate, nextDate, curKqOvertimeRulesDetailEntity, eventMap, preKqOvertimeRulesDetailEntity, restpointmap, index);
            if (signMap != null && !signMap.isEmpty()) {
    ...
    private Map<String, Object> getRestSignInfo(String resourceid, String splitDate, String preDate,
                                                ...
                                                Map<String, Object> eventMap,
                                                KQOvertimeRulesDetailEntity preKqOvertimeRulesDetailEntity, Map<String, Object> restpointmap, int index) {
        ...
        signMap = getNonWorkSignInfo(resourceid, nextDate, splitDate, pre_lsSignTime, next_lsSignTime, curKqOvertimeRulesDetailEntity, eventMap, preKqOvertimeRulesDetailEntity, restpointmap, index);
    ...
    public Map<String, Object> getNonWorkSignInfo(String resourceid, String nextDate, String kqDate,
        ...
        //add
        String from0 = Util.null2String(restpointmap.get("from0"));
        String to0 = Util.null2String(restpointmap.get("to0"));
        String from1 = Util.null2String(restpointmap.get("from1"));
        String to1 = Util.null2String(restpointmap.get("to1"));
        String from2 = Util.null2String(restpointmap.get("from2"));
        String to2 = Util.null2String(restpointmap.get("to2"));
        if (null != restpointmap && !restpointmap.isEmpty()) {
            String count = Util.null2String(restpointmap.get("count"));
            String restpointtime0 = Util.null2String(restpointmap.get("restpoint0"));
            String restpointtime1 = Util.null2String(restpointmap.get("restpoint1"));
            String restpointtime = Util.null2String(restpointmap.get("restpoint" + index));
            if ("1".equals(count)) {//一天一次上下班
                if (index == 0) {
                    signEndDateTime = kqDate + " " + restpointtime0;
                } else {
                    signBeginDateTime = kqDate + " " + restpointtime0;
                    ore_signBeginDateTime = kqDate + " " + restpointtime0;
                }
            } else if ("2".equals(count)) {//一天两次上下班
                if (index == 0) {
                    signEndDateTime = kqDate + " " + restpointtime0;
                } else {
                    signBeginDateTime = kqDate + " " + restpointtime0;
            ...
            } else if ("3".equals(count)) {//一天三次上下班
                if (index == 0) {
                    signBeginDateTime = kqDate + " " + from0 + ":00";
                    signEndDateTime = kqDate + " " + to0 + ":59";
                } else if (index == 1) {
                    signBeginDateTime = kqDate + " " + from1 + ":00";
                    ore_signBeginDateTime = kqDate + " " + restpointtime0;
                    signEndDateTime = kqDate + " " + to1 + ":59";
                } else if (index == 2) {
                    signBeginDateTime = kqDate + " " + from2 + ":00";
                    ore_signBeginDateTime = kqDate + " " + restpointtime1;
                    signEndDateTime = nextDate + " " + to2 + ":59";
                }
            }
        }
        //end
        //如果是第三次上下班  先看看第二次有没有延迟取卡
        String sql = "select * from (" + base_sql + ") a " + " order by signdate asc,signtime asc ";
        boolean isUsed = false;
        if (index == 2){
            rs.executeQuery(sql, resourceid, kqDate + " " + from1 + ":00", kqDate + " " + to1 + ":59");
            if (rs.getCounts()==1){
                sql = "select top 2 * from (" + base_sql + ") a " + " order by signdate asc,signtime asc ";
                rs.executeQuery(sql, resourceid, kqDate + " " + from1 + ":00",nextDate + " " + to2 + ":59" );
                if (rs.getCounts()>1){
                    //代表取了第三次上班的一次卡
                    isUsed  = true;
                }
        ...
        bb.writeLog("10311 ===> isUsed = "+ isUsed);
        ...
        //qc2613259 如果第二次上下班有且只有一次卡 启用延时取卡
        if (index == 1 && rs.getCounts()==1 ){
            sql = "select top 2 * from (" + base_sql + ") a " + " order by signdate asc,signtime asc ";
            rs.executeQuery(sql, resourceid, signBeginDateTime,nextDate + " " + to2 + ":59" );
        }
        ...
        int idx = 0;
        int countTemp = rs.getCounts();
        while (rs.next()) {
            //如果使用过延时取卡  第三次上下班就要跳过第一次被取的卡
            ...
            bb.writeLog("10311 ===> signdate = "+ signdate);
            bb.writeLog("10311 ===> signtime = "+ signtime);
            if (index == 2 && isUsed){
                bb.writeLog("10311 ===> skiped");
                isUsed = false;
                countTemp--;
                continue;
            }
            ...
            idx++;
            bb.writeLog("10314 ===> idx = "+ idx);
            bb.writeLog("10314 ===> countTemp = "+ countTemp);
            if (idx == 1) {//第一条算签到
                checkInfo.put("signType", "1");
                lsCheckInfo.add(checkInfo);
            } else if (idx == countTemp) {//最后一条算签退
        ...
        bb.writeLog("1031 ===> lsCheckInfo = "+ lsCheckInfo);
        return signMap;
    }
