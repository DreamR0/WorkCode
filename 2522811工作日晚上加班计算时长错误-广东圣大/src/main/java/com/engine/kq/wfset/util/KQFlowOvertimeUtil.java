...
public class KQFlowOvertimeUtil {
  ...
  public Map<String,String> handleKQOvertimeAction(Map<String, String> sqlMap,
    ...
    if(!sqlMap.isEmpty()){
      for(Map.Entry<String,String> me : sqlMap.entrySet()){
        ...
        new BaseBean().writeLog("==zj==(value)" + JSON.toJSONString(value));
        rs1.execute(value);
        while (rs1.next()) {
  ...
  public static void bean4Overtime(String prefix, RecordSet rs1, SplitBean splitBean) {
    ...
    String serial_type = Util.null2s(rs1.getString(prefix+"serial_type"), "-1");
    new BaseBean().writeLog("==zj==(serial_type)" + JSON.toJSONString(serial_type) + "  |  " + prefix+"serial_type");
    ...
    splitBean.setSerial_type(serial_type);
  }
