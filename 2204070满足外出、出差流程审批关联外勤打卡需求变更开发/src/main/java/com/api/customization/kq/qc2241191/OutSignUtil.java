...
public class OutSignUtil {
    private String businessSchedule = Util.null2String(new BaseBean().getPropValue("qc2204070","businessSchedule")); //出差表
    private String outSchedule = Util.null2String(new BaseBean().getPropValue("qc2204070","outSchedule"));           //公出表
    ...
    private String todaysTimes = sdfh.format(l);  //人员打卡时间
    private String todaysDate = sdfd.format(l);   //人员打卡日期

    private String ASD = Util.null2String(new BaseBean().getPropValue("qc2204070","ASD"));    //获取实际开始日期
    private String AST = Util.null2String(new BaseBean().getPropValue("qc2204070","AST"));    //获取实际开始时间
    private String AED = Util.null2String(new BaseBean().getPropValue("qc2204070","AED"));    //获取实际结束日期
    private String AET = Util.null2String(new BaseBean().getPropValue("qc2204070","AET"));    //获取实际结束时间
    ...
    public Map<String, String> OutSignCheck(String resourceId){
        ...
        String businessSingBaseSql = "select  a.id,b.requestid,b.currentnodetype from (select top 1 * from " + outSchedule + " where "+resourceIdSchedule+"="+resourceId+" order by id desc) a left join workflow_requestbase b on b.requestid=a.requestId";
        String businessSingWhereSql = " where '" +todaysDate+todaysTimes+"' between a."+ASD +"+"+ "a."+AST+" and a."+AED+"+"+"a."+AET+" order by id desc";
        String businessSingSql = businessSingBaseSql +businessSingWhereSql;
        new BaseBean().writeLog("==zj==(公出考勤验证sql)" + businessSingSql);
        rs.executeQuery(businessSingSql);
        if (rs.next()){
                //改变需求，如果流程有就可以开始打卡，打卡范围在预计日期范围
                //满足流程归档和流程提交范围时间内，进入此方法，校验是否满足当天班次打卡时间范围
                timeMap = OutSignTimeCheck(resourceId, todaysDate, todaysTimes);
                new BaseBean().writeLog("==zj==公出流程（打卡Map）" + JSON.toJSONString(timeMap));
        ...
        //如果公出匹配成功，则不需要执行出差流程，避免数据覆盖
        if (!result){
            String outSingBaseSql = "select  a.id,b.requestid,b.currentnodetype from (select top 1 * from " + businessSchedule + " where "+resourceIdSchedule+"="+resourceId+" order by id desc) a left join workflow_requestbase b on b.requestid=a.requestId";
            String outSingWhereSql = " where '" +todaysDate+todaysTimes+"' between a."+ASD +"+"+ "a."+AST+" and a."+AED+"+"+"a."+AET+" order by id desc";
            String outSingSql = outSingBaseSql +outSingWhereSql;
            new BaseBean().writeLog("==zj==(出差考勤验证sql)" + outSingSql);
