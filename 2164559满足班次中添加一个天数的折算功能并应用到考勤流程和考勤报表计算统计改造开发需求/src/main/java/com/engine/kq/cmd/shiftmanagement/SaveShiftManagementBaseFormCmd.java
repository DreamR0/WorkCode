...
public class SaveShiftManagementBaseFormCmd extends AbstractCommonCommand<Map<String, Object>>{
  ...
  public void edit(Map<String,Object> retmap,JSONObject jsonObj) throws Exception{
    new BaseBean().writeLog("==zj==（编辑班次进入）");
...
//    String color = Util.null2o(Util.null2String(jsonObj.get("color")));//工作时长
    ...
    //==zj 自定义班次基本信息--折算时长开关、折算时长
    String conversioncross = Util.null2s(jsonObj.getString("conversioncross"),"0");//是否开启折算天数开关：0-不开启、1-开启。默认不开启
    String conversiondays = Util.null2s(jsonObj.getString("conversiondays"),"0");//设置折算天数：默认为0天，并且保留两位小数

    Double conversiondaysD = Double.parseDouble(conversiondays);  //将折算天数转为double类型
    conversiondays = String.format("%.2f",conversiondaysD);  //折算天数保留两位小数
    //==
    ...
    String mainSql = "update kq_ShiftManagement set serial=?,subcompanyid=?,shiftonoffworkcount=?,punchsettings=?,isoffdutyfreecheck=?,isresttimeopen=?,worktime=?,color=?," +
        "cardRemind=? ,cardRemOfSignIn=? ,minsBeforeSignIn=? ,cardRemOfSignOut=? ,minsAfterSignOut=? ,remindMode=? ,remindOnPC=?,halfcalrule=?,halfcalpoint=?,halfcalpoint2cross=?,conversioncross=?,conversiondays=? where id = ? ";
    ...
    boolean isUpdated = rst.executeUpdate(mainSql, serial,subcompanyid,shiftOnOffWorkCount,punchSettings,isOffDutyFreeCheck,isRestTimeOpen,worktime,color,
        cardRemind ,cardRemOfSignIn ,minsBeforeSignIn ,cardRemOfSignOut ,minsAfterSignOut ,remindMode ,remindOnPC,halfcalrule,halfcalpoint,halfcalpoint2cross,conversioncross,conversiondays,serialid);
  ...
  public void add(Map<String,Object> retmap,JSONObject jsonObj) throws Exception{
    new BaseBean().writeLog("==zj==（新增班次进入）");
    RecordSetTrans rst = new RecordSetTrans();
    rst.setAutoCommit(true);
    RecordSet rs = new RecordSet();
    String subcompanyid = Util.null2o(Util.null2String(jsonObj.get("subcompanyid")));
...
//    String color = Util.null2o(Util.null2String(jsonObj.get("color")));//工作时长
    ...
    String halfcalpoint2cross = Util.null2s(jsonObj.getString("halfcalpoint2cross"),"0");//当日
    //==zj==自定义班次基本信息form组件
    String conversioncross = Util.null2s(jsonObj.getString("conversioncross"),"0");//折算时长开关：0-关闭、1-开启。默认不开启
    String conversiondays = Util.null2s(jsonObj.getString("conversiondays"),"0");//自定义折算天数。默认为0天。

    //==zj==将折算天数转化为Double类型，并保留两位小数
    Double conversiondaysD = Double.parseDouble(conversiondays);
    conversiondays = String.format("%.2f",conversiondaysD);  //==zj==折算天数保留两位小数
...
//    String color = getRandomColor();
    ...
    String mainSql = "insert into kq_ShiftManagement(serial,subcompanyid,shiftonoffworkcount,punchsettings,isoffdutyfreecheck,isresttimeopen,worktime,uuid,color," +
        "cardRemind ,cardRemOfSignIn ,minsBeforeSignIn ,cardRemOfSignOut ,minsAfterSignOut ,remindMode ,remindOnPC,halfcalrule,halfcalpoint,halfcalpoint2cross,conversioncross,conversiondays)"
        + " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

    //==zj==班次基本信息表
    new BaseBean().writeLog("==zj==（班次基本信息）" + mainSql);
    new BaseBean().writeLog("==zj==(折算开关--折算天数)" + conversioncross + " | " + conversiondays);
