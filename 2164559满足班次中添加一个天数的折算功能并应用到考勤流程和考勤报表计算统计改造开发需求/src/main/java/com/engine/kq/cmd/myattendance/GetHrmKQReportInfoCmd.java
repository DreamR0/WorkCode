    ...
    public Map<String, Object> execute(CommandContext commandContext) {
        ...
        try {
            ...
            for (int i = 0; datasList != null && i < datasList.size(); i++) {
                ...
                new BaseBean().writeLog("==zj==(获取应出勤天数)" + workDays);
                realWorkDays += Util.getDoubleValue((String) datasMap.get("attendancedays"), 0.00);//实际出勤天数
                new BaseBean().writeLog("==zj==(获取实际出勤天数)" +realWorkDays);
            ...
            new BaseBean().writeLog("==zj==(统计应出勤天数判断)" + !toDate.equals("") + " | "+toDate.compareTo(currentDate)+" | "+Util.getIntValue(resourceComInfo.getStatus(resourceId),0));
            if (!toDate.equals("") && toDate.compareTo(currentDate) > 0 && Util.getIntValue(resourceComInfo.getStatus(resourceId), 0) < 4) {
                ...
                for (String date = fromDate; !isEnd; ) {
                    ...
                    boolean isWorkDay = kqWorkTime.isWorkDay(resourceId, date); // 判断当天是否是工作日

                    if (isWorkDay) {
                        double conversiondays = 0.0;
                        String serialIds = kqWorkTime.getSerialIds(resourceId, date);   //==zj 判断当天班次信息
                        String sql = "select * from kq_ShiftManagement where id ="+serialIds;
                        rs.executeQuery(sql);
                        if (rs.next()){
                            conversiondays = Util.getDoubleValue(rs.getString("conversiondays"));
                        }
                        if (conversiondays == 1.00 || conversiondays == 0.00){
                            workDays += 1.00;
                        }else {
                            workDays += conversiondays;
                        }
                ...
                new BaseBean().writeLog("==zj==(workDays被重新计算了)" + workDays);
            }
