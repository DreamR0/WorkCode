...
public class GetShiftManagementBaseFormCmd extends AbstractCommonCommand<Map<String, Object>>{
  ...
  public Map<String, Object> execute(CommandContext commandContext) {
    ...
    try {
      ...
      //配置折算时长开关的标签id
      String conversionid = Util.null2String(new BaseBean().getPropValue("qc2164559","fieldid"));
      String conversiondaysid = Util.null2String(new BaseBean().getPropValue("qc2164559","countdaysid"));
      ...
      String[] fields = new String[]{"serial,125818,1,1","subcompanyid,141,3,169","shiftonoffworkcount,388563,5,3","punchsettings,388564,4,1",
          ...
          "isoffdutyfreecheck,388568,4,2","halfcalpoint,513145,3,19","halfcalpoint2cross,513090,5,1","conversioncross,"+conversionid+",4,2","conversiondays,"+conversiondaysid+",1,1"};

      new BaseBean().writeLog("==zj==（班次基本信息组件）" + JSON.toJSONString(fields));
      ...
      if(_id.length() > 0){
        ...
        new BaseBean().writeLog("==zj==(getShiftInfo)" + getShiftInfo);
        new BaseBean().writeLog("==zj==(_id)" + _id);
            ...
            if("shiftonoffworkcount".equalsIgnoreCase(fieldname) ||
              ...
              fieldvalue = Util.null2s(fieldvalue, "1");
                ...
                "halfcalpoint2cross".equalsIgnoreCase(fieldname) ||
                    //==zj==将初始值设置为0
                "conversioncross".equalsIgnoreCase(fieldname) ||
                 "conversiondays".equalsIgnoreCase(fieldname)){
