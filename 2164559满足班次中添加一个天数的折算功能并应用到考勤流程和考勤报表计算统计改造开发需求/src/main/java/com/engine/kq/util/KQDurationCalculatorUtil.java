...
//   * 1-公众假日、2-工作日、3-休息日
      ...
      new BaseBean().writeLog("==zj==(加班splitBean)" + JSON.toJSONString(splitBean));
      new BaseBean().writeLog("==zj==(加班splitBeans)"+JSON.toJSONString(splitBeans));
      if("3".equalsIgnoreCase(durationrule) || "5".equalsIgnoreCase(durationrule) || "6".equalsIgnoreCase(durationrule)){
        ...
        new BaseBean().writeLog("==zj==（加班计算）" + "Min_Duration--"+Min_Duration + " | oneDayHour--" + oneDayHour + " | d_day--" + d_day);
  ...
  public Map<String,Object> getWorkDuration(){
    ...
    try{
      ...
      new BaseBean().writeLog("==zj==(splitBean)" + JSON.toJSONString(splitBean));
