  ...
  public Map<String, Object> formatKqDate(String userId, String kqDate) {
    ...
    try {
      ...
      if (workTime == null || workTime.getWorkMins() == 0) {
        ...
        //==zj==取出lsParam里面的数据
        new BaseBean().writeLog("==zj==(lsParam)" + JSON.toJSONString(lsParam));
        List<Object> list = lsParam.get(0);       //获取数据数组
        String serialid = (String) list.get(3);   //获取数组班次id

        //==zj==获取该班次对应的开关状态和折算时长
        String conversioncross = "";  //初始化开关状态
        Double conversiondays = 1.00;   //初始化折算时长

        String conversionsql = "select * from kq_ShiftManagement where id='" + serialid + "'";
        new BaseBean().writeLog("==zj==(kq_format_total--折算时长sql)" + conversionsql);
        RecordSet rw = new RecordSet();
        rw.executeQuery(conversionsql);
        if (rw.next()){
          conversioncross = Util.null2String(rw.getString("conversioncross"));
          //==zj==如果开关为开启状态
          if (conversioncross.equals("1")){
            conversiondays = rw.getDouble("conversiondays");
          }
        }
        new BaseBean().writeLog("==zj==(kq_format_total--折算时长)" + conversiondays);
        ...
        if (lsParam.size() > 0) {
          ...
          //==zj  实际出勤天数折算（then后面删除1*）因为数据库workdays不支持小数点值，所以更换字段类型
          ...
          sql = " insert into kq_format_total(resourceid,kqdate,subcompanyid,departmentid,jobtitle,groupid,serialid,workdays,workmins," +
              ...
              " select a.resourceid,kqdate,b.subcompanyid1,b.departmentid,b.jobtitle,groupid,serialid," +
              " case when sum(workmins)>0 then "+conversiondays +  " end as workdays,sum(workmins) as workmins," +
              " cast(sum(attendanceMins)AS decimal(10, 2))/sum(workmins)*"+conversiondays+ "as attendancedays, sum(attendanceMins) as attendanceMins," +
              ...
              new BaseBean().writeLog("==zj==(kq_format_total)" + sql);
          rs.executeUpdate(sql, userId, kqDate);
        }
      }
    }catch (Exception e) {
