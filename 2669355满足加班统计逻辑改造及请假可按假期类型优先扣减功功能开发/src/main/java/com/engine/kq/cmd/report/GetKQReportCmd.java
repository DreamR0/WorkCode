...
public class GetKQReportCmd extends AbstractCommonCommand<Map<String, Object>> {
	...
	public Map<String, Object> execute(CommandContext commandContext) {
		...
		String BalanceOfLeaveTable = new BaseBean().getPropValue("qc2669355","BalanceOfLeaveTable");//抵扣调休事假表
		String typeId = "leaveType_"+new BaseBean().getPropValue("qc2669355","typeId");//事假类型id
		String tiaoxiuId = "leaveType_"+ new BaseBean().getPropValue("qc2669355","tiaoxiuId");//调休类型id
...
//			orderBy = "order by "+orderBy;
          ...
          //==z 将抵扣的事假转为调休时长
					String leaveDeduction = leaveCustomUtil.getLeave(id, fromDate, toDate);//获取抵扣的事假时长
					Double leaveDeductionD = Double.parseDouble(leaveDeduction);
					Double flowLeaveDataD = Double.parseDouble(flowLeaveData);

					if (typeId.equals(flowType)){
						//如果是事假，就减去抵扣调休的事假时长
						flowLeaveData =Util.null2String(flowLeaveDataD - leaveDeductionD <= 0 ? 0:flowLeaveDataD - leaveDeductionD) ;
					}

					if (tiaoxiuId.equals(flowType)){
						flowLeaveData = Util.null2String(leaveDeductionD);
					}
