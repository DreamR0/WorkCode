...
public class KqReportUtil {
    ...
    public String resignationCal(String resourceId, String fromDate,String toDate) {
        ...
        String resourceEnd = "";
        String sql = "";
        String  resignationSql = "";
        try {
            sql = "select * from hrmresource where id=" + resourceId;
            new BaseBean().writeLog("==zj==(入离职缺勤判断sql)" + sql);
            rs.executeQuery(sql);
            if (rs.next()) {
                //获取该人员账号的创建日期，以及离职日期
           ...
           //入职前,先将1号到创建日期的数据查出来,然后看是否在查询范围内
            String createStart = resourceCreate.substring(0,7) + "-01";
            sql = "select * from kq_shiftschedule where resourceid = "+resourceId+" and kqdate between '"+createStart+"' and '"+resourceCreate+"'"+
            " and kqdate not in('"+resourceCreate+"')";
              resignationSql = "select count(*) as days from ("+sql+")[table] where kqdate between '"+fromDate+"' and '"+toDate+"'";
            new BaseBean().writeLog("==zj==(入职前sql)" + resignationSql);
            rs.executeQuery(resignationSql);
            if (rs.next()){
                days += rs.getInt("days");
            }
            new BaseBean().writeLog("==zj==(计算查询范围内当月入职前考勤天数)" + days);

            if (!"".equals(resourceEnd)){
                //这里计算离职之后当月的缺勤时长
                //计算离职当月一共多少天
                int year = Integer.parseInt(resourceEnd.substring(0,4));
                int month = Integer.parseInt(resourceEnd.substring(5,7));
                Calendar calendar = Calendar.getInstance();
                //设置年份月份
                calendar.set(Calendar.YEAR,year);
                calendar.set(Calendar.MONTH,month);
                //获取当月最后一天
                String lastDays = calendar.getActualMaximum(Calendar.DAY_OF_MONTH)+"";
                String resourceToEnd = resourceEnd.substring(0,7)+lastDays;
                sql = "select * from kq_shiftschedule where resourceid = "+resourceId+" and kqdate between '"+resourceEnd+"' and '"+resourceToEnd+"'"+
                        " and kqdate not in('"+resourceEnd+"')";
                resignationSql = "select count(*) as days from ("+sql+")[table] where kqdate between '"+fromDate+"' and '"+toDate+"'";
                rs.executeQuery(resignationSql);
                if (rs.next()){
                    days += rs.getInt("days");
                }
                new BaseBean().writeLog("==zj==(离职之后当月的缺勤天数)" + days);
            }
