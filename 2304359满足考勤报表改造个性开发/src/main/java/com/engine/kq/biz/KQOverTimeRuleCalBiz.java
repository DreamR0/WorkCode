...
//                    }
  ...
  public Map<String,String> buildOvertimeCard(KQOvertimeCardBean kqOvertimeCardBean,
    ...
    new BaseBean().writeLog("==zj==(signinTime-signoutTime)" + signinTime + " | " + signoutTime);
    ...
    new BaseBean().writeLog("==zj==(纯打卡-signinTime)" + signinTime);
    //先把打卡数据都列出来 置位1
    if(signinTimeIndex < signoutTimeIndex){
    ...
    int changeType = kqReportUtil.getChangeType(resourceid,splitDate);
    new BaseBean().writeLog("==zj==(纯打卡加班--加班类型)" + changeType);
    //==zj 1-节假日 2-工作日 3-休息日
    if (changeType == 1 || changeType == 3){
      String overTimeRest = new BaseBean().getPropValue("qc2304359","overtimeStart");
      new BaseBean().writeLog("==zj==(休息日加班)" + overTimeRest + " | " + signinTime);
      //如果是节假日或者休息日就从08:15开始计算加班
      new BaseBean().writeLog("==zj==(比较时间)" + overTimeRest.compareTo(signinTime));
      if (overTimeRest.compareTo(signinTime) >= 1){
        //说明开始打卡时间在规定时间之前，把规定之前的时间就覆盖了
        int overTimeRestIndex = kqTimesArrayComInfo.getArrayindexByTimes(overTimeRest);
        new BaseBean().writeLog("==zj==(signinTimeIndex--overTimeRestIndex)"+signinTimeIndex+" | " + overTimeRestIndex);
        Arrays.fill(initArrays, signinTimeIndex, overTimeRestIndex,-2);
      }
    }
        ...
        if(restTimeType == 1){
          for(int j =0 ; j < restTimeList.size() ; j++){
            ...
            if(restTimes.length == 2){
              ...
              new BaseBean().writeLog("==zj==(纯打卡加班休息时间)" + restStart + " | " + restEnd);
  ...
  public void clearWorkAndRestTime(String resourceid, String splitDate, boolean isNextDay,
    ...
    new BaseBean().writeLog("==zj==(respurceid)" + resourceid + " | " + splitDate);
