    ...
    try {
      ...
      if (workTime == null || workTime.getWorkMins() == 0) {
        ...
        if(!nonlsParam.isEmpty()){
          sql = " insert into kq_format_detail(resourceid,kqdate,groupid,serialnumber,signindate,signintime,signinid,signoutdate,signouttime,signoutid,leaveMins,leaveinfo,evectionMins,outMins,lunchAllowance,dinnerAllowance,midnightAllowance)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
          ...
          sql = " insert into kq_format_total(resourceid,kqdate,subcompanyid,departmentid,jobtitle,groupid,serialid,workdays," +
              ...
              " graveleaveearly,graveleaveearlymins,absenteeism,absenteeismmins,forgotcheck, forgotcheckMins,leaveMins,evectionMins,outMins,lunchAllowance,dinnerAllowance,midnightAllowance)" +
              ...
              " sum(leaveMins) as leaveMins, sum(evectionMins) as evectionMins, sum(outMins) as outMins" +
                  ",sum(lunchAllowance) as lunchAllowance," +"sum(dinnerAllowance) as dinnerAllowance," +"sum(midnightAllowance) as midnightAllowance"+
        ...
        //==zj qc2304359 detail表和total表 添加一个新字段（lunchAllowance）
        if (lsParam.size() > 0) {
          sql = " insert into kq_format_detail( " +
              ...
              " leaveMins,leaveinfo,evectionMins,outMins,forgotbeginworkcheckmins,otherinfo,lunchAllowance,dinnerAllowance,midnightAllowance"+(definedField.length()>0?","+definedField+"":"")+") " +
          ...
          sql = " insert into kq_format_total(resourceid,kqdate,subcompanyid,departmentid,jobtitle,groupid,serialid,workdays,workmins," +
              ...
              " leaveMins,evectionMins,outMins,forgotbeginworkcheck,forgotbeginworkcheckmins,lunchAllowance,dinnerAllowance,midnightAllowance"+(definedField.length()>0?","+definedField+"":"")+") " +
              " select a.resourceid,kqdate,b.subcompanyid1,b.departmentid,b.jobtitle,groupid,serialid," +
              " case when sum(workmins)>0 then 1 end as workdays, sum(workmins) as workmins," +
...
//          System.out.println("fieldname=="+fieldname);
        ...
        //==zj qc2304359 工作日午餐补助（出勤时长>4小时）
        int lunchAllowance =  0;
        if (attendanceMins > 0){
          Double attendanceHour =attendanceMins/60.0;
          if(attendanceHour > 4){
            lunchAllowance = 1;
          }
        }
        params.add(lunchAllowance);

        //==zj qc2304359 工作日晚餐补助（当天下班打卡时间超过20:00  ，计入一次，每天最多一次（当天没有打下班卡的，不计入晚餐补助））
        int dinnerAllowance =  0;
        new BaseBean().writeLog("==zj==(signOutTime)" +signOutTime + " | " +  signOutTime.compareTo("19:30:00"));
        if (!"".equals(signOutTime)){
          if (signOutTime.compareTo("19:30:00") >= 1){
            //如果超过20:00,晚餐补助生效
            dinnerAllowance = 1;
          }
        }
        params.add(dinnerAllowance);

        //==zj qc2304359 工作日夜宵补助（当天下班打卡时间超过22:30-6:30计入一次，每天最多一次（当天没有打下班卡的，不计入夜宵补助））
        int midnightAllowance = 0;
        if (!"".equals(signOutTime)){
        String kqDateTime = kqDate + "22:30:00";  //当天22:30:00开始
        String kqDateTimeNext = kqReportUtil.DateCal(kqDate)+"06:30:00";//隔天06:30:00结束
        String signOutDateTime = signOutDate + signOutTime;
        new BaseBean().writeLog("==zj==(夜宵补助)" + kqDateTime + " | " + signOutDateTime + " | "+kqDateTimeNext);
          //当天22:30 到明天 06:30 之间
          if (signOutDateTime.compareTo(kqDateTime) >= 1 && signOutDateTime.compareTo(kqDateTimeNext) <= 0){
            midnightAllowance = 1;
          }
        }
        params.add(midnightAllowance);
  ...
  public void formatNonWork(String userId, String kqDate, List<Object> nonlsParam, WorkTimeEntity workTime, Map<String, Object> workFlowInfo) {
    ...
    KqReportUtil kqReportUtil = new KqReportUtil();
    //==zj qc2304359 工作日午餐补助（出勤时长>4小时）
    double kqHour = kqReportUtil.kqMins(signInDate,signInTime,signOutDate,signOutTime);
    int lunchAllowance =  0;
      if(kqHour > 4){
        lunchAllowance = 1;
      }
    nonlsParam.add(lunchAllowance);

    //==zj qc2304359 工作日晚餐补助（当天下班打卡时间超过20:00  ，计入一次，每天最多一次（当天没有打下班卡的，不计入晚餐补助））
    int dinnerAllowance =  0;
    if (!"".equals(signOutTime)){
      String dinnerKqTime = signOutDate+signOutTime;
      String dinnerKqTimeStart = kqDate + "20:00:00";
      if (dinnerKqTime.compareTo(dinnerKqTimeStart) >= 1){
        //如果超过20:00,晚餐补助生效
        dinnerAllowance = 1;
      }
    }
    nonlsParam.add(dinnerAllowance);
    ...
    nonlsParam.add(midnightAllowance);
