...
//			orderBy = "order by "+orderBy;
			...
			new BaseBean().writeLog("==zj==(考勤汇总报表sql)" + sql);
			rs.execute(sql);
			while (rs.next()) {
				data = new HashMap<>();
              ...
              fieldValue = KQDurationCalculatorUtil.getDurationRound(businessLeave);
						...
						String workmins = Util.null2String(rs.getString("workmins"));	//应出勤分钟数
						if (!"".equals(workmins)){
							String workminsLeave = kqReportUtil.leaveCount(flowData,id);	//扣减调休时长
							new BaseBean().writeLog("==zj==(扣减后应工作时长公式)"+"应出勤小时-年假出差调休时长:" + workmins + " - " + workminsLeave);
					...
					}else if ("leaveEarlyMins".equals(fieldName)){
						//早退时长以0.5h为单位向上取整,每条记录取整完再累加
						fieldValue = kqReportUtil.getAbsence(id,fromDate,toDate,fieldName);
					}else if("beLateMins".equals(fieldName)){
						//迟到时长以0.5h为单位向上取整
						fieldValue = kqReportUtil.getAbsence(id,fromDate,toDate,fieldName);
						...
						new BaseBean().writeLog("==zj==(事假总计时长)" + fieldValue);
						...
						//计算午餐补助
						fieldValue = Util.null2String(rs.getString("lunchAllowance"),"0");
					}else if ("dinnerAllowance".equals(fieldName)){
						//计算晚餐补助
						fieldValue = Util.null2String(rs.getString("dinnerAllowance"),"0");
					}else if ("midnightAllowance".equals(fieldName)){
						//计算夜宵补助
						fieldValue = Util.null2String(rs.getString("midnightAllowance"),"0");
					}else {
