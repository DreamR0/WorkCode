    ...
    try {
      ...
      if(rs.next()){
        ...
        if(kqtype == KqSplitFlowTypeEnum.LEAVE.getFlowtype()){
          ...
          //add
          proc_set_id = rs.getString("id");
          //得到这个考勤流程设置是否使用明细
          usedetails = rs.getString("usedetail");
          maintablename = rs.getString("tablename");
          detailtablename = rs.getString("detailtablename");
          if(temprequestid > 0){
            map.put("requestId", "and t.requestId = " + temprequestid);
          }
          //end
        ...
        if(tablename.length() > 0){
          ...
          if(isCard){
            ...
            if(detailtablename.length()>0){
              ...
              Map<String, String> sqlMap = handleSql(proc_set_id, usedetails, temprequestid,kqtype,map);
              ...
              DateTimeFormatter datetimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");
              Map<String,String> result=delCardAction(sqlMap,splitBeans,datetimeFormatter,Util.getIntValue(workflowid),temprequestid,rci);

            }

            //下面的逻辑有性能问题，在流程补卡数据的是采用的额like模糊匹配，进行全表扫表，在hrmschedulesign表数据量较大的时候，退回流程有性能问题
...
//            }
            ...
            if(isLeave) {
              for(SplitBean splitBean : backsplitBeans){
                ...
                kqLog.info("流程退回后主动返还假期:resourceId:"+resourceId+":duration:"+duration+":newLeaveType:"+newLeaveType+":durationrule:"+durationrule+":requestid:"+requestid+":fromdatedb:"+fromdatedb);
  ...
  public Map<String,String> delCardAction(Map<String, String> sqlMap,
                                          List<SplitBean> splitBeans, DateTimeFormatter datetimeFormatter, int workflowId,
    ...
    if(!sqlMap.isEmpty()){
      for(Map.Entry<String,String> me : sqlMap.entrySet()){
        ...
        while (rs1.next()) {
          ...
          StringBuffer sql = new StringBuffer("delete from HrmScheduleSign where userid='").append(userId).append("' and signdate='").append(signDate).append("'")
                  .append(" and signtime='").append(signTime).append("' and signfrom='").append(signfrom).append("'");
          kqLog.info("delcardsql="+sql.toString());
          boolean isOk = rs2.execute(sql.toString());

          if(!isOk){
            result.put("status", "-1");
            result.put("message", "删除补卡数据失败！");
            return result;
          }
          kqLog.info("delCardAction:userId:"+userId+":signDate:"+signDate+":signTime:"+signTime);

          String format_key = userId+"_"+signDate;
          formateList.add(format_key);
        }

      }
      kqLog.info("KQFlowActiontBiz isCard delTest:formateList:"+formateList);
      if(null != formateList && formateList.size()>0){
        for(String formateStr : formateList){
          String[] formateStrs = formateStr.split("_");
          new KQFormatData().formatKqDate(formateStrs[0],formateStrs[1]);
        }
