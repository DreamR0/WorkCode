      ...
      if (workTime == null || workTime.getWorkMins() == 0) {
        ...
        if (lsParam.size() > 0) {
          ...
          sql = " insert into kq_format_detail( " +
                  ...
                  " leaveMins,leaveinfo,evectionMins,outMins,forgotbeginworkcheckmins,otherinfo,earlyshift,nightshift"+(definedField.length()>0?","+definedField+"":"")+") " +
          ...
          sql = " insert into kq_format_total(resourceid,kqdate,subcompanyid,departmentid,jobtitle,groupid,serialid,workdays,workmins," +
                  ...
                  " leaveMins,evectionMins,outMins,forgotbeginworkcheck,forgotbeginworkcheckmins,earlyshift,nightshift"+(definedField.length()>0?","+definedField+"":"")+") " +
                  " select a.resourceid,kqdate,b.subcompanyid1,b.departmentid,b.jobtitle,groupid,serialid," +
                  " case when sum(workmins)>0 then 1 end as workdays, sum(workmins) as workmins," +
          ...
          new BaseBean().writeLog("==zj==(total)" + sql);
          rs.executeUpdate(sql, userId, kqDate);
        }
      }
    }catch (Exception e) {
...
//          System.out.println("fieldname=="+fieldname);
        ...
        double earlyshift = 0;
        double nightshift = 0;
        String sqlSerialId = "select * from kq_ShiftManagement where id=" + serialId;
        new BaseBean().writeLog("==zj==(查询班次基本信息)" + sqlSerialId);
        rs.executeQuery(sqlSerialId);
        if (rs.next()) {
          String serial = Util.null2String(rs.getString("serial"));
          new BaseBean().writeLog("==zj==(考勤班次名称)" + serial);

          new BaseBean().writeLog("==zj==(班次信息匹配)"+serial + " | " + "早".equals(serial) + " || "+"夜".equals(serial));
          if ("早".equals(serial)){
            earlyshift = kqUtil.shiftCount(attendanceMins);
            new BaseBean().writeLog("==zj==(早班)" + earlyshift+" | "+nightshift);
            params.add(earlyshift);
            params.add(nightshift);
          }else if ("夜".equals(serial)){
            nightshift = kqUtil.shiftCount(attendanceMins);
            new BaseBean().writeLog("==zj==(晚班)" + earlyshift+" | "+nightshift);
            params.add(earlyshift);
            params.add(nightshift);
          }else {
            nightshift=0.0;
