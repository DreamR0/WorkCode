...
//		}
    ...
    public static boolean checkUserRight(String processname, User user, String departmentid) {
      if(user.getLoginid().equalsIgnoreCase("sysadmin")) return true ;
		//qc2474652 这里如果是人力资源修改，权限判断根据建模表
		if("HrmResourceEdit:Edit".equals(processname)){
			HrmVarifyCustom hrmVarifyCustom = new HrmVarifyCustom();
			String department = hrmVarifyCustom.getDepartment(user, false);
			if (department.contains(departmentid)) {
				return true;
			}
		}
      new BaseBean().writeLog("==zj==(processname)" + JSON.toJSONString(processname));
      new BaseBean().writeLog("==zj==(departmentid)" + JSON.toJSONString(departmentid));
      CheckUserRight ck = new CheckUserRight() ;
