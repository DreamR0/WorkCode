...
//                address = addressTemp;
        ...
        RecordSet rsTypes = new RecordSet();
        rsTypes.executeQuery("select id from Meeting_Type mt where isEnableSecondAuth = 1 order by id");
        while(rsTypes.next()){
            typeIds.add(Util.null2String(rsTypes.getString("id")));
        }
        ret.put("isEnableSecondAuthTypes",typeIds);
...
//                            }
                        ...
                        if(fieldtype == 2){
                            ...
                            new BaseBean().writeLog("==zj==(会议浏览按钮)" + fieldlabel + " | " + fieldname + " | " + fieldtype);
...
//                            }
                        ...
                        if ("remindImmediately".equals(fieldname)) {
                            ...
                            item = FieldUtil.getRemindItem(fieldname, SystemEnv.getHtmlLabelNames(fieldlabel, user.getLanguage()), listValue, viewAttr);
                        }else if("isEnableSecondAuth".equals(fieldname)){
                            RecordSet rs = new RecordSet();
                            rs.executeQuery("select * from enc_secondauth_config_info where mouldcode =? and itemcode=? ","MEETING","NODE");
                            if(fieldValue.equals("")){
                                fieldValue = "0";
                            }
                            if(typeIds.contains(meetingtype)){
                                fieldValue = "1";
                            }
                            if(rs.next()){
                                String isenable = rs.getString("isenable");
                                if("1".equals(isenable)){
                                    item = FieldUtil.getFormItemForSwitch(fieldname,SystemEnv.getHtmlLabelNames(fieldlabel, user.getLanguage()),fieldValue,typeIds.contains(meetingtype)? 1:viewAttr);
                                }else{
                                    continue;
                                }
                            }
...
//                        item = MeetingUtil.getCommonAccessoryInfo(user,meetingid,meetingtype,fieldname,fieldlabel,viewAttr,fieldValue,ismobile,"");
                    ...
                    if("isEnableSecondAuth".equals(fieldname)){
                        if(isChange || isEdit){
                            itemlist.add(item);
                        }
                    }else{
