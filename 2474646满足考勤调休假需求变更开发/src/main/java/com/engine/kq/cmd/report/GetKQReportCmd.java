...
//			orderBy = "order by "+orderBy;
			...
			while (rs.next()) {
				...
				while (kqReportFieldComInfo.next()){
					...
					new BaseBean().writeLog("==zj==(fieldName)" +fieldName);
					String fieldValue = "";
              ...
              fieldValue = KQDurationCalculatorUtil.getDurationRound(businessLeave);
						...
						//qc2474646 调休余额根据时间范围显示
						BalanceOfLeaveUtil balanceOfLeaveUtil = new BalanceOfLeaveUtil();
						fieldValue = balanceOfLeaveUtil.getBalanceOfLeave(id,fromDate,toDate,kqLeaveRulesBiz, flowData);
						new BaseBean().writeLog("==zj==(fieldValue)" + fieldValue);
						/*fieldValue= KQBalanceOfLeaveBiz.getRestAmount(id, leaveRulesId, Datetime);*/
					} else {
