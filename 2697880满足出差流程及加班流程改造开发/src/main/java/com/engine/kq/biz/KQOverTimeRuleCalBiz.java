...
//        需审批，以打卡为准，取流程和打卡的交集
  ...
  private void doComputingMode4(String resourceid, String splitDate,
   ...
   /* //==zj 这里如果有出差流程加班就以流程为主不看打卡
    ...
    Boolean isBusiness = customUtil.isBusiness(resourceid,splitDate);
    new BaseBean().writeLog("==zj==(需审批，取交集-出差标识)" + isBusiness);
    new BaseBean().writeLog("==zj==(splitBeanMaps)" + JSON.toJSONString(splitBeanMaps1));
    String Key = resourceid+"_"+splitDate;
    if (splitBeanMaps1.containsKey(Key) && isBusiness){
      List<SplitBean> splitBeans = splitBeanMaps1.get(Key);
      String flowinfo = "";
      if(splitBeans != null && !splitBeans.isEmpty()){
        ...
        logOvertimeMap(overtimeLogMap, "打卡数据KQOvertimeCardBean为null", Key+"|"+"打卡和上下班数据|KQOvertimeCardBean");
      }
      eventMap.put(Key+"|"+"加班流程数据|flowinfo", flowinfo);
      for (int i = 0; i < splitBeans.size(); i++) {
        SplitBean splitBean = splitBeans.get(i);
        new BaseBean().writeLog("==zj==(需审批，取交集splitBean)" + JSON.toJSONString(splitBean));
        if ("".equals(splitBean.getBelongDate())){
          splitBean.setBelongDate(splitDate);
        }
        customUtil.doComputingMode1_splitBean(splitBean);
      }
    }*/
    ...
    while (iterator.hasNext() /*&& !isBusiness*/) {
      ...
      if(kqOvertimeCardBean != null){
        ...
        //==zj 流程打卡交集 如果“是否统计休息时长”打开，则计算休息时长
        Map<String, String> signinoffMap = null;
        SplitBean bean = null;
        if(splitBeanMaps.containsKey(mapKey)){
          List<SplitBean> splitBeans = splitBeanMaps.get(mapKey);
          if (splitBeans != null){
             bean = splitBeans.get(0);
          }
        }
        if (bean != null && "1".equals(bean.getIsOpenRest())){
          signinoffMap = customUtil.buildOvertimeCard(kqOvertimeCardBean, resourceid, realSplitDate, kqTimesArrayComInfo, restTimeMap, changeType_key,initArrays,hasOverTime4SignList,
                  overRulesDetailMap,true,overtimeLogMap);
        }else {
          signinoffMap = buildOvertimeCard(kqOvertimeCardBean, resourceid, realSplitDate, kqTimesArrayComInfo, restTimeMap, changeType_key,initArrays,hasOverTime4SignList,
                  overRulesDetailMap,true,overtimeLogMap);
        }
...
//                }
  ...
  private void doComputingMode2(String resourceid, String splitDate,
    ...
    /*//==zj 这里如果有出差流程加班就以流程为主不看打卡
    Boolean isBusiness = customUtil.isBusiness(resourceid,splitDate);
    new BaseBean().writeLog("==zj==(需审批，以打卡为准-出差标识)" + isBusiness);
    new BaseBean().writeLog("==zj==(splitBeanMaps)" + JSON.toJSONString(splitBeanMaps));
    String Key = resourceid+"_"+splitDate;
    if (splitBeanMaps.containsKey(Key) && isBusiness){
      List<SplitBean> splitBeans = splitBeanMaps.get(Key);
      String flowinfo = "";
      if(splitBeans != null && !splitBeans.isEmpty()){
        flowinfo = JSON.toJSONString(splitBeans, SerializerFeature.DisableCheckSpecialChar,SerializerFeature.DisableCircularReferenceDetect);
      }else{
      ...
      for (int i = 0; i < splitBeans.size(); i++) {
        SplitBean splitBean = splitBeans.get(i);
        new BaseBean().writeLog("==zj==(需审批，以打卡为准splitBean)" + JSON.toJSONString(splitBean));
        if ("".equals(splitBean.getBelongDate())){
          splitBean.setBelongDate(splitDate);
        }
        customUtil.doComputingMode1_splitBean(splitBean);
...
//                生成过加班，但是其实只用了1小时，还需要拿这个加班流程来，减去已经用的1小时，d_mins就是这个加班流程实际可用的加班流程时长
            ...
            if(d_mins > 0){
              ...
              int[] initArrays = kqTimesArrayComInfo.getInitArr();
              //==zj 如果“是否统计休息时长”打开，则计算休息时长
              Map<String, String> signinoffMap = null;
              if ("1".equals(splitBean.getIsOpenRest())){
                signinoffMap = customUtil.buildOvertimeCard(kqOvertimeCardBean, resourceid, realSplitDate, kqTimesArrayComInfo, restTimeMap, changeType_key,initArrays,hasOverTimeList,
                        overRulesDetailMap, false, overtimeLogMap);
              }else {
                 signinoffMap = buildOvertimeCard(kqOvertimeCardBean, resourceid, realSplitDate, kqTimesArrayComInfo, restTimeMap, changeType_key,initArrays,hasOverTimeList,
                        overRulesDetailMap, false, overtimeLogMap);
              }
