...
public class ExportUtil {
    ...
    public ExportUtil(Map<String, Object> params, HttpServletRequest request, HttpServletResponse response, User user){
        this.user = user;
        this.params = params;
...
//      }
            ...
            if (isconfirm.length() > 0){
                String resourceIds = checkUtil.isConfirm(fromDate);
                if ("0".equals(isconfirm)){
                    //未确认
                    if (resourceIds.length() > 0){
                        sqlWhere += " and a.id not in("+resourceIds+") ";
                    }
                }
                if ("1".equals(isconfirm)){
                    ...
                    if (resourceIds.length() <= 0){
                        ...
                        sqlWhere += " and a.id  in("+resourceIds+") ";
                    }
                }
            }
            ...
            new BaseBean().writeLog("==zj==(工时lsSheet)" + JSON.toJSONString(sheet));
            lsSheet.add(sheet);
