...
public class WorkingKqReportUtil {
    private Map<String, Object> params = new HashMap<>();
    private User user = null;

    public WorkingKqReportUtil(Map<String, Object> params, User user) {
        this.user = user;
        this.params = params;
    }
    ...
    public Map<String, Object> getKqReport(){
        ...
        try{
            ...
            //qc20230816
            WorkingCheckUtil workingCheckUtil = new WorkingCheckUtil();
            while (kqReportFieldComInfo.next()){
                ...
                if (!workingCheckUtil.isShow(kqReportFieldComInfo.getFieldname()))continue;
            ...
            rs.execute(sql);
            new BaseBean().writeLog("==zj==(工时报表sql)" + JSON.toJSONString(sql));
            while (rs.next()) {
                data = new HashMap<>();
                ...
                //qc20230816 这里把日历状态显示每天上班工时
                CalendarUtil calendarUtil = new CalendarUtil();
                Map<String,Object> detialDatas = calendarUtil.getDetialDatas(id,fromDate,toDate,user,false);
...
//            new KQLog().info("id:date:"+(id+"|"+date)+":detialDatas.get:"+detialDatas.get(id+"|"+date));
    ...
    private List<Object>  getCascadeKeyColumnsInfo(String cascadeKey, User user){
        ...
        for(int i=0;i<lsCascadeKey.size();i++){
            kqReportFieldComInfo.setTofirstRow();
            while (kqReportFieldComInfo.next()) {
                new BaseBean().writeLog("==zj==(考勤个人确认表)" + JSON.toJSONString(kqReportFieldComInfo.getFieldname()));
                if(!kqReportFieldComInfo.getReportType().equals("month"))continue;
