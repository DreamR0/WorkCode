	...
	public Map<String, Object> execute(CommandContext commandContext) {
		...
		try{
		  ...
		  DepartmentComInfo departmentComInfo = new DepartmentComInfo();
			...
			new BaseBean().writeLog("==zj==(columns)" + JSON.toJSONString(columns));
			boolean isEnd = false;
			...
			while (rs.next()) {
              ...
              if ("officialBusiness".equals(fieldName)){
				  ...
				  fieldValue = kqFormatUtil.halfDayCount(fieldValue);
							...
							//qc20230816 这里对不调休加班进行显示折算
							if ("workingDayOvertime_nonleave".equals(fieldName) || "restDayOvertime_nonleave".equals(fieldName) || "holidayOvertime_nonleave".equals(fieldName)){
								fieldValue = kqReportUtil.overtimeObversion(id,fieldName,fromDate,toDate);
							}
							new BaseBean().writeLog("==zj==(加班折算)" + id+ " | " + fieldName + "  |  " + fieldValue);
						}
					} else {
