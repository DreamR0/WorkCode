  ...
  public Map<String,Object> getDetialDatas(String resourceId,String fromDate, String toDate, User user){
    ...
    try {
      sql = " select resourceid, kqdate, workMins, belatemins, graveBeLateMins, leaveearlymins, graveLeaveEarlyMins, absenteeismmins, forgotcheckMins, forgotBeginWorkCheckMins, "+
            " leaveMins,leaveInfo,evectionMins,outMins,serialid " +
            " from kq_format_detail " +
            " where resourceid = ? and kqdate>=? and kqdate<=? "+
...
//  }
  ...
  public int getSerialCount(String resourceId, String fromDate, String toDate, String serialId){
    ...
    int leaveCount = 0;
    try{
      sql = "select count(1) from hrmresource a, kq_format_total b where a.id= b.resourceid and b.resourceid = ? and b.kqdate >=? and b.kqdate <=? and b.serialId = ? and b.leaveMins=0 and b.absenteeismmins = 0 and b.attendancemins > 0";
      rs.executeQuery(sql,resourceId,fromDate,toDate, serialId);
      if(rs.next()){
        serialCount = rs.getInt(1);
      }
    /*  //qc20230816 如果当天有请假就不算出勤班次
      sql = "select count(leaveinfo) as leavecount from kq_format_detail where resourceid = ? and kqdate >=? and kqdate <=? and serialId = ? and leaveinfo not in('{}')";
      rs.executeQuery(sql,resourceId,fromDate,toDate,serialId);
      if (rs.next()){
        leaveCount = rs.getInt("leavecount");
      }
      new BaseBean().writeLog("==zj==(leavecount)" + JSON.toJSONString(leaveCount));
      serialCount -= leaveCount;*/
    }catch (Exception e){
      writeLog(e);
    }
    return serialCount;
  ...
  public void reflow(String requestid){
    try {
      ...
      String sql = "select requestid,workflowid from workflow_requestbase where requestid = '"+requestid+"'";
