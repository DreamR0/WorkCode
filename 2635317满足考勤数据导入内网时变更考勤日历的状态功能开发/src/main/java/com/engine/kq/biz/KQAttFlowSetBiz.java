...
//      }
  ...
  public Map<String,String> getFLowSql(Map<String, Object> params, User user){
    ...
    String sqlWhere = "";
    //qc2635317 如果不是请假和加班明细，走标准
   new BaseBean().writeLog("==zj==(外层1kqtype)" + JSON.toJSONString(kqtype));
    if (kqtype == 0 || kqtype == 3){
      sqlWhere = " where 1=1 and requestidcustom > 0 ";
    }else {
      sqlWhere = " where 1=1 and req_requestid > 0 ";
    }
  ...
  private String getFinishedByKQType(String kqtype, String fromDate, String toDate, User user,
    ...
    if(isLeave){
      ...
      String backSql = "select sum(duration) from "+back_table_name+"  where "+back_table_name+".leavebackrequestid="+table_name+".requestidcustom and "+back_table_name+".newleavetype="+table_name+".newleavetype and "+back_table_name+".resourceid="+table_name+".resourceid ";
      if(fromDate.length() > 0 && toDate.length() > 0){
        if("1".equalsIgnoreCase(isMyKQ)){
      ...
      fields = " distinct requestid,requestidcustom,resourceid as resourceId,subcompanyid,departmentid,newleavetype,fromdatedb as fromDate,fromtimedb as fromTime,todatedb as toDate,totimedb as toTime,durationdb,durationrule,SUM(duration) as duration,"
    ...
    baseSql += " GROUP BY requestid,requestidcustom,resourceid,subcompanyid,departmentid,newleavetype,fromdatedb,fromtimedb,todatedb,totimedb,durationdb,durationrule ";

    String unionsql = " select u.*,w.currentnodetype,w.requestname,w.requestid as req_requestid,COALESCE(w.status, '外部导入') as flowstatus,w.workflowid,a.managerid,a.managerstr,a.loginid,a.subcompanyid1 as asubcompanyid1,a.departmentid as adepartmentid,a.lastname from ("+baseSql+") u left join Workflow_Requestbase w on u.requestid = w.requestid  left join hrmresource a on u.resourceid = a.id ";
    new BaseBean().writeLog("==zj==(内网请假明细sql)" + JSON.toJSONString(unionsql));
    return unionsql;
  }
  ...
  public String buildFlowSetTableString(Map<String, Object> params, User user){
    ...
    new BaseBean().writeLog("==zj==(内网backfields)" + backfields+ fromSql+ sqlWhere);
