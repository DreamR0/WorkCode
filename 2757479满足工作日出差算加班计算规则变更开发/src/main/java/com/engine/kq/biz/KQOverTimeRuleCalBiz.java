...
import com.api.customization.kq.biz.KqFlexBiz;
import com.api.customization.qc2757479.util.KqUtil;
    ...
    while (iterator.hasNext()) {
      ...
      if(kqOvertimeCardBean != null){
        ...
        if(signinTimeIndex < signoutTimeIndex){
          ...
          //==z 如果是当天是出差且为工作日 加班时长 = 打卡时长-7.5
          if (kqUtil.isBusiness(resourceid,splitDate)){
            if (splitBeanMaps.containsKey(mapKey)){
              List<SplitBean> splitBeans = splitBeanMaps.get(mapKey);
              for (int o = 0; o < splitBeans.size(); o++) {
                SplitBean splitBean = splitBeans.get(o);
                kqUtil.buildCustomOverTime(resourceid,splitBean,kqOvertimeCardBean,splitDate,realSplitDate,kqTimesArrayComInfo,changeType,kqOvertimeRulesDetailEntity);
              }
            }
          }
          //当天没有出差流程走标准
          if(over_count > 0 && !kqUtil.isBusiness(resourceid,splitDate)){
            int restTimeType = 1;
...
//                    }
  ...
  public void clearWorkAndRestTime(String resourceid, String splitDate, boolean isNextDay,
    ...
    if (workTime == null || workTime.getWorkMins() == 0 || workTime.getIsRestSerial() == 1) {
    }else{
  ...
  public Map<String, Object> getNonWorkSignInfo(String resourceid, String nextDate, String kqDate,
    ...
    //如果当天没有出差流程，且非特殊排除人员，则加班排除外勤卡
    if (!KqFlexBiz.isEvection(resourceid,kqDate) && !KqFlexBiz.isNoEvection(resourceid)){
      base_sql += " and signfrom <> 'e9_mobile_out' ";
    }
  ...
  public Map<String, Object> getSignInfo(String userId, TimeScopeEntity signTimeScope,
    ...
    //如果当天没有出差流程，且非特殊排除人员，则加班排除外勤卡
    if (!KqFlexBiz.isEvection(userId,kqDate) && !KqFlexBiz.isNoEvection(userId)){
      base_sql += " and signfrom <> 'e9_mobile_out' ";
    }
