...
//        按照加班时长转调休的 时长设置
        ...
        if(curKqOvertimeRulesDetailEntity != null){
          ...
          if(overtime_cut_point.length() == 0){
            ...
            WorkTimeEntity workTime = new KQWorkTime().getWorkTime(resourceid, splitDate);
            if (workTime.getIsRestSerial() == 1){
                TimeScopeEntity signscope = workTime.getSignTime().get(0);
                if (signscope.getEndTimeAcross()){
                    overtime_cut_point = signscope.getEndTime();
                }
            }
        }else{
          continue;
...
//        1-节假日、2-工作日、3-休息日
          ...
          WorkTimeEntity workTime = new KQWorkTime().getWorkTime(resourceid, preSplitDate);
          if (workTime.getIsRestSerial() == 1){
                TimeScopeEntity signscope = workTime.getSignTime().get(0);
                if (signscope.getEndTimeAcross()){
                    pre_overtime_cut_point = signscope.getEndTime();
                }
          }
          if(changeType == 2){
          ...
          if(i == betweenDays){
              if (toTime.compareTo(pre_overtime_cut_point) > 0) {
  ...
  public boolean handle_changeType_2(int[] initArrays, String resourceid, String splitDate,
    ...
    if(cur_shiftInfoBean != null){
      ...
      if(real_workLongTimeIndex != null && !real_workLongTimeIndex.isEmpty()){
        ...
        isBusiness = kqUtil.isBusiness(resourceid,splitDate);
        if (isBusiness){
          new BaseBean().writeLog("==zj==(不排除工作日)");
          Arrays.fill(initArrays, fromTime_index,toTime_index,0);
        }
