...
<html>
	...
	<%
			...
			if("publicAll".equals(loadFolderType)){//公共目录
			   if("1".equals(subDocscribe)){
			    ...
			    if("myShare".equals(loadFolderType)){
			    	...
			    	if(categoryid == 0){
				    	operateString += "     <operate otherpara=\"column:type\" href=\"javascript:doShareObj()\"  text=\"" + SystemEnv.getHtmlLabelName(390469,user.getLanguage()) + "\" index=\"0\"/>";
				    ...
				    operateString += "     <operate otherpara=\"column:type\" href=\"javascript:doShareFn()\"   text=\"" + "分享" + "\" index=\"6\"/>";
				}
			    operateString += "</operates>";
			}
			...
			if("myShare".equals(loadFolderType) || "shareMy".equals(loadFolderType)){
			    timeLabel = SystemEnv.getHtmlLabelName(391245,user.getLanguage());
			}else{
		    ...
		    if("shareMy".equals(loadFolderType)){
                if(categoryid == 0 || !txt.isEmpty()){
    	            tableString += "<col width=\"" + userCol + "\" labelid=\"26805\"  text=\"" + SystemEnv.getHtmlLabelName(126836,user.getLanguage())  + "\" column=\"username\" />";
    	            ...
    	            tableString += "<col width=\"" + userCol + "\" labelid=\"26805\"  text=\""+ SystemEnv.getHtmlLabelName(126836,user.getLanguage())  + "\" otherpara=\"" + username + "\" column=\"shareid\" transmethod=\"weaver.docs.rdeploy.util.SplitPageForRdeploy.getCommonValue\"/>";
	...
	<body>
		...
		<div id="dataloading" style="text-align:center;position: absolute;left: 45%;top: 20%;display:none;">
						<img src="<%=weaver.general.GCONST.getContextPath()%>/rdeploy/assets/img/doc/loading.gif">
					</div>
	...
	<script>
		...
		function fullPrivateItemData(categoryid,itemData,params){
			...
			if(loadFolderType == "publicAll" && para.length > 0){
				getAllParents(categoryid,function(){
					location.href = "<%=weaver.general.GCONST.getContextPath()%>"+_href;
				});
			}else{
				location.href = "<%=weaver.general.GCONST.getContextPath()%>"+_href;
			}	
		...
		function getAllParents(categoryid,fn){
			...
			jQuery.ajax({
				url : "<%=weaver.general.GCONST.getContextPath()%>/rdeploy/chatproject/doc/eventForAjax.jsp",
				type : "post",
		...
		function doRename(){
			 ...
			 var _name = $obj.html();
			 var _extname = "";
			 if(_name.indexOf(".") > -1 && $obj.hasClass("file")){
			 ...
			 $input.blur(function(){
			 	...
			 	var that = this;
			 	if(_name == ""){
		    		top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(81968,user.getLanguage())%>",function(){
		    			jQuery(that).focus();
		    		});
		    		return;
		    	}else if(/[\\/:*?"<>|]/.test(_name)){129176
		    		top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(129176,user.getLanguage())%>:<br/>\\/:*?\"<>|",function(){
		    			jQuery(that).focus();
		    		});
		    		return;
		    	}
				...
				var url = "";
				if($obj.hasClass("file")){
					url = "<%=weaver.general.GCONST.getContextPath()%>/rdeploy/chatproject/doc/renameImageFile.jsp"
					data = {
						fileName : _name,
						imagefileid : _id,
						uid : _uid,
					...
					url = "<%=weaver.general.GCONST.getContextPath()%>/rdeploy/chatproject/doc/addSeccategory.jsp"
					data = {
		...
		function doOpen(id){
			...
			if($obj.hasClass("file")){
				if(parent.jQuery("#loadFolderType").val() == "publicAll"){
					parent.openUrl("<%=weaver.general.GCONST.getContextPath()%>/docs/docs/DocDsp.jsp?id=" + id);
				}else{
					var shareid = "";
					...
					if(parent.window.__isBrowser){
						parent.openFullWindowForXtable("<%=weaver.general.GCONST.getContextPath()%>/rdeploy/chatproject/doc/imageFileView.jsp?fileid=" + id + "&shareid=" + shareid);
					}else{
						parent.openUrl("<%=weaver.general.GCONST.getContextPath()%>/rdeploy/chatproject/doc/imageFileView.jsp?fileid=" + id + "&shareid=" + shareid);
					}
				}
		...
		function doEditDoc(docid){
			parent.openUrl("<%=weaver.general.GCONST.getContextPath()%>/docs/docs/DocEdit.jsp?id=" + docid);
		...
		function doDelDoc(docid){
			window.top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(30952,user.getLanguage())%>?",function(){
	        	var url = "<%=weaver.general.GCONST.getContextPath()%>/docs/docs/DocOperate.jsp?operation=delete&docid="+docid;
	        	jQuery.ajax({
	        		url : url , 
					data : {},
					url : url ,
		...
		function doShareDoc(docid){
			...
			dialog.URL = "<%=weaver.general.GCONST.getContextPath()%>/docs/tabs/DocCommonTab.jsp?_fromURL=46&isdialog=1&id=" + docid;
		...
		function doShowLog(docid){
			...
			dialog.Height = 500;
			dialog.checkDataChange = false;
			dialog.URL = "<%=weaver.general.GCONST.getContextPath()%>/docs/DocDetailLogTab.jsp?_fromURL=0&isdialog=1&id=" + docid;
			dialog.maxiumnable = true;
			dialog.show();
		}
