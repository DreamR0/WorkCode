...
import com.api.cusomization.kq.OverTimeUtli;
...
//        需审批，以打卡为准，取流程和打卡的交集
  ...
  private void doComputingMode4(String resourceid, String splitDate,
    ...
    String lastworkday = "";
    int count = 1;
    Map<String,Object> restpointmap = getrestpoint(resourceid,splitDate,dateFormatter);
...
//        return;
      ...
      while (iterator.hasNext()) {
        ...
        if (kqOvertimeCardBean != null) {
          ...
          new BaseBean().writeLog("==zj==（上下班打卡时间）" + signinTime + "--" + signoutTime);
          String signinDate = Util.null2String(signinoffMap.get("signinDate"));
          String signoutDate = Util.null2String(signinoffMap.get("signoutDate"));

          new BaseBean().writeLog("==zj==（上下班日期）" + signinDate + "--" + signoutDate);
          int signinTimeIndex = kqTimesArrayComInfo.getArrayindexByTimes(signinTime);
          int signoutTimeIndex = kqTimesArrayComInfo.getArrayindexByTimes(signoutTime);
          String flow_cross_key = mapKey + "|" + "加班计算区间|" + signinTime + "-" + signoutTime;

          new BaseBean().writeLog("==zj==(flow_cross_key)" + flow_cross_key);
          if (signinTimeIndex < signoutTimeIndex) {
            int over_count = kqTimesArrayComInfo.getCnt(initArrays, signinTimeIndex, signoutTimeIndex, 1);
            String countLogInfo = "signinTimeIndex:" + signinTimeIndex + ":signoutTimeIndex:" + signoutTimeIndex + ":over_count:" + over_count;
            new BaseBean().writeLog("==zj==(countLogInfo)" +countLogInfo);
            ...
            new BaseBean().writeLog("==zj==（人员id和当前时间）" + resourceid + "--" + endDate);
            ...
            String overTimeIn = "";//加班上班卡
            String overTimeOut = "";//加班下班卡
            ...
            new BaseBean().writeLog("==zj==（查询加班后打卡次数sql）" + overTimesql);
            //查询下班之后打卡次数
            ...
            if (countSign < 3){
              over_count = 0;
                ...
                String overInSql = "select top 1 * from(" + "select top 2 signTime,signDate from hrmschedulesign "+
                                  ...
                                  "[table]";
                new BaseBean().writeLog("==zj==(获取加班上班卡sql)"+ overInSql);
              ro.executeQuery(overInSql);
                if (ro.next()){
                  overTimeIn = Util.null2String(ro.getString("signTime"));  //获取加班上班卡时间
                }

              //==zj==计算加班时长
              overTimeOut = signoutTime;  //获取加班下班卡时间
              new BaseBean().writeLog("==zj==(加班计算-时间)" + "加班开始时间-" + overTimeIn + " | 加班结束时间-" + overTimeOut);
              int overTimeinIndex = kqTimesArrayComInfo.getArrayindexByTimes(overTimeIn); //加班上班卡转换分钟数
              int overTimeoutIndex = kqTimesArrayComInfo.getArrayindexByTimes(overTimeOut); //加班下班卡转换分钟数
              over_count = overTimeoutIndex - overTimeinIndex;
              new BaseBean().writeLog("==zj==(加班计算-分钟)" +"加班时长-" + over_count + " | 加班开始时间:" + overTimeinIndex + " | 加班下班时间:" + overTimeoutIndex);
            }
...
//                生成过加班，但是其实只用了1小时，还需要拿这个加班流程来，减去已经用的1小时，d_mins就是这个加班流程实际可用的加班流程时长
              ...
              if (d_mins > 0) {
                ...
                if (signinTimeIndex < signoutTimeIndex) {
                  ...
                  //==zj 计算下班之后，有几次打卡次数。
                  OverTimeUtli otu = new OverTimeUtli();
                  Map<String,Object> resultMap = otu.overTimeSignCardCheck(resourceid,signinDate,signoutDate);
                  int over_count = 0;   //系统标准加班分钟数
                  int over_countNon = otu.mapChangeCheck(resultMap.get("over_countNon"));  //下班到加班上班卡之间的时间
                  boolean result = otu.mapChangeCheck(resultMap.get("result")); //检查下班之后是否满足三次打卡

                  if (result){
                     over_count = kqTimesArrayComInfo.getCnt(initArrays, signinTimeIndex, signoutTimeIndex, 1);
                     over_count -= over_countNon;
                     new BaseBean().writeLog("==zj==(第二种方式--打卡加班时长)" + over_count + " | "+over_countNon);
                  }
                  //
                  new BaseBean().writeLog("==zj==(第二种方式--最终打卡生成的时长)" + over_count);
                  int card_mins = over_count;
...
//                    }
  ...
  public Map<String, Object> getrestpoint(String resourceid, String belongDate,DateTimeFormatter dateFormatter){
    ...
    if (curWorkTime == null || curWorkTime.getWorkMins() == 0) {//当前是休息日或者节假日
      KQTimesArrayComInfo kqTimesArrayComInfo = new KQTimesArrayComInfo();
      ...
      overtimemap.put("from0","08:00");
      overtimemap.put("to0","12:00");
      overtimemap.put("from1","13:00");
      overtimemap.put("to1","17:30");
      overtimemap.put("from2","17:30");
      overtimemap.put("to2","03:59");
  ...
  public Map<String, Object> getNonWorkSignInfo(String resourceid, String nextDate, String kqDate,
    ...
    if(null != restpointmap && !restpointmap.isEmpty()){
      ...
      if("1".equals(count)){//一天一次上下班
        ...
        if(index == 0){
          signBeginDateTime = kqDate+" "+card_from0+":00";
          signEndDateTime = kqDate+" "+card_to0+":59";;
        }else if(index == 1){
          signBeginDateTime = kqDate+" "+card_from1+":00";
          ore_signBeginDateTime = kqDate+" "+restpointtime0+":00";
          signEndDateTime = kqDate+" "+card_to1+":59";
        }else if(index == 2){
          signBeginDateTime = kqDate+" "+card_from2+":00";;
          ore_signBeginDateTime = kqDate+" "+restpointtime1;
          signEndDateTime = nextDate+" "+card_to2+":59";
        }
      }
    }
    //end
