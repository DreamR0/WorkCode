...
public class KqReportUtil {
    ...
    public Map<String, Object> getKqReport(){
        ...
        try{
            ...
            new BaseBean().writeLog("==zj==(分部id)" + subCompanyId);
            ...
            new BaseBean().writeLog("==zj==(分页和数量)" + pageIndex+" | "+pageSize);
...
//        new KQLog().info("id:"+id+":detialDatas:"+detialDatas);
                ...
                String kqGroupId="";
                String customSql = " select DISTINCT groupid  from kq_format_total where resourceid='"+id+"' and kqdate between '"+fromDate+"' and '"+toDate+"'";
                new BaseBean().writeLog("==zj==(考勤组获取)" + customSql);
                rsCustom.executeQuery(customSql);
                while (rsCustom.next()){
                    kqGroupId += Util.null2String(rsCustom.getString("groupid"))+",";
                }
