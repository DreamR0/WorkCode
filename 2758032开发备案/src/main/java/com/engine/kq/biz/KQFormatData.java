...
public class KQFormatData extends BaseBean {
  ...
  public List<List<Object>> format(String userId, String kqDate, WorkTimeEntity workTime,
    ...
    try {
      ...
      int[] countDayMins = new int[2880];//用来计算指定请假流程分钟数
      ...
      for (int i = 0; lsWorkTime != null && i < lsWorkTime.size(); i++) {
        ...
        //==z
        int leaveCustomTotal = 0;
        String travelWorkRestid =new BaseBean().getPropValue("qc2758032","travelWorkRestid");
        String noAllowanceAttendanceid =new BaseBean().getPropValue("qc2758032","noAllowanceAttendanceid");
        ...
        for (int j = 0; workFlow != null && j < workFlow.size(); j++) {
          ...
          if(flowType.equals(FlowReportTypeEnum.EVECTION.getFlowType())){
            ...
            if (endIdx > beginIdx) {
              ...
              //==z
              if ("21".equals(userId)){
                new BaseBean().writeLog("==zj==(newLeaveType)" + newLeaveType + " | " + beginIdx + " | " + endIdx);
              }
              new BaseBean().writeLog("==zj==(travelWorkRestid)" + travelWorkRestid + " | " + noAllowanceAttendanceid);
              if (travelWorkRestid.equals(newLeaveType) || noAllowanceAttendanceid.equals(newLeaveType)){
                Arrays.fill(countDayMins,beginIdx,endIdx,1);
              }
          ...
          for (int j = 0; workFlow != null && j < workFlow.size(); j++) {
            ...
            if(flowType.equals(FlowReportTypeEnum.EVECTION.getFlowType())){
              ...
              if (endIdx > beginIdx) {
                Arrays.fill(dayMins, beginIdx, endIdx, 5);//流程抵扣时段标识 5
                //==z
                if ("13".equals(travelWorkRestid) || "14".equals(noAllowanceAttendanceid)){
                  Arrays.fill(countDayMins,beginIdx,endIdx,1);
                }
              }
            }else{
      ...
      //==z
        for (int k = workBeginIdx; k < workEndIdx; k++) {
          switch (countDayMins[k]) {
            case 1:
              leaveCustomTotal++;
              break;
          }
        ...
        if ("21".equals(userId)){
          new BaseBean().writeLog("==zj==(leaveCustomTotal)" + leaveCustomTotal );
        }
        ...
        //==z 这里不扣减路途工休班和无不出出勤的请假时长
        attendanceMins = workMins - absenteeismMins-leaveMins-beLateMins-graveBeLateMins-leaveEarlyMins-graveLeaveEarlyMins + leaveCustomTotal;
