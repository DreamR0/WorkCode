...
import com.engine.kq.cmd.attendanceEvent.GetOverTimeWorkDurationCmd;
...
public class KQBalanceOfLeaveBiz {
    ...
    public static String addExtraAmountByDis5(String resourceId, String date, String durationOfOvertime, String type, String hoursOfworkday, String requestId, String fromCardOrFlow, String fromDateDb,Map<String,Object> otherParam) {
        ...
        try {
            ...
            userId =  GetOverTimeWorkDurationCmd.threadLocal.get();
            new BaseBean().writeLog("==zj==(调休userid)" + JSON.toJSONString(userId));
            if (!"1".equals(userId) && userId != null/*&& false*/){
                CheckUtil checkUtil = new CheckUtil();
                reusltMap = checkUtil.getOverSet(userId);
                new BaseBean().writeLog("==zj==(reusltMap调休)" + JSON.toJSONString(reusltMap));
            }
            if (reusltMap.size() > 0){
                minimumUnitOfOvertime =(int) reusltMap.get("minimumUnit");
...
//              1-按加班时长比例转调休时长、2-按加班时长范围设置转调休时长、3-按加班的时间段设置转调休时长
    ...
    public static boolean updateExtraAmountByDis5(String balanceOfLeaveId, double durationOfOvertime, String requestId) {
        ...
        try {
            ...
            new BaseBean().writeLog("==zj==(hoursToDay2)" + hoursToDay);
