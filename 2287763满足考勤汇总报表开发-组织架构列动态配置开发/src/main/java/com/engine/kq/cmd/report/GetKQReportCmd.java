		  ...
		  DepartmentComInfo departmentComInfo = new DepartmentComInfo();
			...
			while (kqReportFieldComInfo.next()){
				...
				if ("workdays".equals(kqReportFieldComInfo.getFieldname())){
					...
					kqReportUtil.getColumn(columns);
				}
				column = new HashMap();
...
//			orderBy = "order by "+orderBy;
			...
			while (rs.next()) {
				...
				new BaseBean().writeLog("==zj==(kqReportFieldComInfo--更新)" +JSON.toJSONString(kqReportFieldComInfo));
				...
				while (kqReportFieldComInfo.next()){
					...
					if(fieldName.equals("subcompany")){
						...
						//==zj 获取部门上级id
						KqReportUtil kqReportUtil = new KqReportUtil();
						List<Map<String,String>> depList = kqReportUtil.getSupDepId(tmpDepartmentId);
						kqReportUtil.getDatas(data,depList);
                    	...
                    	|| kqReportFieldComInfo.getParentid().equals("overtime_4leave") || fieldName.equals("businessLeave") || fieldName.equals("officialBusiness") || fieldName.equals("business1") || fieldName.equals("business2")) {
              ...
              fieldValue = KQDurationCalculatorUtil.getDurationRound(businessLeave);
					...
					}/* else if (fieldName.equals("zbcs")) {//中班次数
