...
import com.api.customization.kq.reportfiled.util.KqReportFieldUtil;
...
public class ExportExcelCmd extends AbstractCommonCommand<Map<String, Object>> {
  ...
  public Map<String, Object> execute(CommandContext commandContext) {
    ...
    try {
      ...
      KqReportFieldUtil kqReportFieldUtil = new KqReportFieldUtil();
      ...
      while (kqReportFieldComInfo.next()) {
        ...
        new BaseBean().writeLog("==zj==(fieldName)" + kqReportFieldComInfo.getFieldname());
        ...
        if (!kqReportFieldComInfo.getReportType().equals("all") && !kqReportFieldComInfo.getReportType().equals("month"))
          continue;
        if ("workdays".equals(kqReportFieldComInfo.getFieldname())){
          //==zj 这里添加汇总报表层级自定义字段
          KqReportUtil kqReportUtil = new KqReportUtil();
          List<String> colList = kqReportUtil.getColList();   //获取汇总报表自定义层级字段
          if (colList != null){
            for (String colKey : colList){
              if (showColumns.contains(colKey)){
                //如果汇总报表层级字段有显示
                this.lsFieldDataKey.add(colKey);
                title = new HashMap<>();
                String colName = kqReportUtil.getcolName(colKey);
                title.put("rowSpan",3);
...
//      }
      ...
      new BaseBean().writeLog("==zj==(titleList)" + JSON.toJSONString(titleList));
      ...
      String backFields = " a.id,a.lastname,a.workcode,a.dsporder,b.resourceid,a.subcompanyid1 as subcompanyid,a.departmentid,a.jobtitle," +
              ...
              " sum(b.absenteeismMins) as absenteeismMins, sum(zbcs) as zbcs,sum(ybcs) as ybcs, sum(b.forgotCheck)+"+forgotBeginWorkCheck_field+" as forgotCheck "+(definedFieldSum.length()>0?","+definedFieldSum+"":"");
      ...
      while (rs.next()) {
        ...
        for (int fieldDataKeyIdx =0;fieldDataKeyIdx<lsFieldDataKey.size();fieldDataKeyIdx++) {
          ...
          if (fieldName.equals("subcompany")) {
            ...
            continue;
          } else if (fieldName.equals("business1") || fieldName.equals("business2")) { // 国内往返出差次数，及国内住宿出差次数
              fieldValue = KQDurationCalculatorUtil.getDurationRound(Util.null2s(Util.null2String(flowData.get(id + "|" + fieldName)), "0.0"));
          } else if (fieldName.equals("restAnnual")){//当前剩余年假
            fieldValue = kqReportFieldUtil.getAnnualRestAmount(id,toDate);
          } else if (fieldName.equals("restTxiaoxiu")) {//当前剩余调休
            fieldValue = kqReportFieldUtil.getTiaoxiuRestAmount(id,toDate);
          }else if(kqReportUtil.getColList().contains(fieldName)){
            //如果fieldName属于自定义层级字段
            String tmpDepartmentId = Util.null2String(rs.getString("departmentid"));
            List<Map<String, String>> supList = kqReportUtil.getSupDepId(tmpDepartmentId);  //获取该人员部门id的所有上级部门
            fieldValue = kqReportUtil.getDatas(fieldName,supList);
          } else {
            fieldValue = Util.formatMultiLang(Util.null2String(rs.getString(fieldName)),""+user.getLanguage());
...
//          Map<String, Object> detialDatas = kqReportBiz.getDetialDatas(id, fromDate, toDate, user,flowData,true);
      ...
      new BaseBean().writeLog("==zj==(dataList)" + JSON.toJSONString(dataList));
